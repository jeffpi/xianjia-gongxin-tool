...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    if base_file and os.path.exists(base_file):
        ...
        default_base = '工信部业务指标_呼叫详情_整理.xlsx'
        if not os.path.exists(default_base):
            log(f"未找到基准文件，跳过基准对齐（正式使用正常）", 53)
            has_base = False
            base = pd.DataFrame()
        else:
            try:
                base = pd.read_excel(default_base, sheet_name='电信').sort_values('开始时间')
                if base is None or len(base) == 0:
                    has_base = False
                else:
                    # 检查基准日期与MMF数据日期是否匹配
    ...
    with xlsxwriter.Workbook(out, {'strings_to_numbers': False, 'nan_inf_to_errors': False, 'default_date_format': 'yyyy-mm-dd hh:mm:ss'}) as wb:
        # ===== 预定义 formats =====
        ...
        # ===== 详细过程 =====
        ws = wb.add_worksheet('数据业务-详细过程')
        cols = list(df_detail.columns)
        ...
        for i in range(n_detail):
            if cancel_check and i > 0 and i % 1000 == 0 and cancel_check():
                raise KeyboardInterrupt('用户取消')
            row0 = i + 1
            d_orig = df_detail.iat[i, 3] if n_col_detail > 3 else None
            e_orig = df_detail.iat[i, 4] if n_col_detail > 4 else None
            d_dash = _is_eo(d_orig)
            e_dash = _is_eo(e_orig)
            has_fi = any(_ne(df_detail.iat[i, c]) for c in range(5, min(9, n_col_detail)))
            for ci in range(n_col_detail):
                if ci == 0:
                    v = 0 if d_dash else df_detail.iat[i, 0]
                    _ws_write(ws, row0, ci, v)
                elif ci == 3:
                    if d_dash:
                        ws.write_string(row0, ci, '-')
                    else:
                        fmt = fmt_d if has_fi else fmt_num
                        try:
                            ws.write_number(row0, ci, float(d_orig), fmt)
                        except (ValueError, TypeError):
                            ws.write_string(row0, ci, str(d_orig), fmt)
                elif ci == 4:
                    if e_dash:
                        ws.write_string(row0, ci, '-')
                    else:
                        fmt = fmt_e if has_fi else fmt_num
                        try:
                            ws.write_number(row0, ci, float(e_orig), fmt)
                        except (ValueError, TypeError):
                            ws.write_string(row0, ci, str(e_orig), fmt)
                else:
                    _ws_write(ws, row0, ci, df_detail.iat[i, ci])

        # 列宽 + 隐藏 J~Q (0-based 列 9~16)
        for ci in range(n_col_detail):
            name = cols[ci]
            if ci == 3: name = '下载速率（Mbps）'
            elif ci == 4: name = '上传速率(Mbps)'
            mx = len(str(name))
            for i in range(min(100, n_detail)):
                cv = df_detail.iat[i, ci]
    ...
    with pd.ExcelWriter(out, engine='xlsxwriter') as w:
        ...
        if merge_raw and len(df_raw_out) > 0:
            ...
            for ri in range(len(df_raw_write)):
                if cancel_check and ri % 5000 == 0 and cancel_check():
                    raise KeyboardInterrupt('用户取消')
                row_vals = []
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.14")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("5G用户级公共监控速率统计工具 V2.5.14")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("5G虚拟用户跟踪语音VQI工具 V2.5.14")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            # 输入文件（选择/清空紧挨着，拖入框明显、高度翻倍）
            gf = QGroupBox("输入文件"); gfl = QVBoxLayout(gf)
            gfl.setContentsMargins(4, 4, 4, 4); gfl.setSpacing(2)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.14 &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-19</b></p>"
                "<ul>"
                "<li><b>V2.5.14</b> 数据业务写入从openpyxl改为xlsxwriter逐行写(真实数据19.6MB验证:提速31%约51→35秒、格式全一致、点取消可在写入过程中断);汇总批注cell保持数字类型可排序(放弃超链接跳转,改用批注内'详见详细过程X列Y行'位置提示);映射解析支持换行交替格式(手机号一行ID一行)+同行多列;临时文件+改名稳妥写入(取消不留半截坏文件)</li>\n"
