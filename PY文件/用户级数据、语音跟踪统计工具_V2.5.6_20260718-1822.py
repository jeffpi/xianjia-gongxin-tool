...
def _has_data_file_cols(df):
    """判断DataFrame是否为数据文件（有QCI列或RLC列，无MessageType列）"""
    if _has_message_type_col(df):
        return False
    has_qci = 'QCI' in df.columns
    has_rlc_dl = any('RLC' in c and 'Downlink' in c for c in df.columns)
    has_rlc_ul = any('RLC' in c and 'Uplink' in c for c in df.columns)
    has_time = 'Time' in df.columns
    if has_time and (has_qci or has_rlc_dl or has_rlc_ul):
        return True
    return False

def _read_file_head(f, nrows=3):
    """只读取文件前n行判断列结构，避免读整个文件"""
    low = f.lower()
    if low.endswith('.csv'):
        for enc in ['gbk', 'utf-8', 'latin-1']:
            try:
                ...
                continue
        return pd.read_csv(f, encoding='latin-1', nrows=nrows, on_bad_lines='skip')
    try:
        ...
        try:
            ...
            try:
                ...
                return pd.read_excel(f, engine='xlrd', nrows=nrows)
...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True):
    ...
    # 分类文件：读取每个文件前3行检查是否有MessageType列
    # 有MessageType → 语音文件；没有 → 数据文件
    voice_files = []
    data_files = []
    for f in all_files:
        try:
            df_test = _read_file_head(f, nrows=3)
            has_msg = _has_message_type_col(df_test)
            has_data = _has_data_file_cols(df_test)
            log(f"  文件 {os.path.basename(f)}: {'语音' if has_msg else ('数据' if has_data else '未知')}", 6)
            if has_msg:
    ...
    log(f"  语音文件: {len(voice_files)} 个, 数据文件: {len(data_files)} 个", 8)

    # ===== 读取数据文件（处理数据指标）=====
    data_dfs = []
    data_dl_rates = []  # 所有数据文件的下行速率
    data_ul_rates = []  # 所有数据文件的上行速率
    data_operator_rates = {}  # {op: {dl_rates: [], ul_rates: []}}
    for fi, f in enumerate(data_files):
        log(f"  读取数据文件 {fi+1}/{len(data_files)}: {os.path.basename(f)}", 10 + int(5 * (fi+1) / max(len(data_files), 1)))
        ...
        # 提取速率数据
        dl_cn = [c for c in df_i.columns if 'RLC' in c and 'Downlink' in c]
        ul_cn = [c for c in df_i.columns if 'RLC' in c and 'Uplink' in c]
        if dl_cn:
            dl_vals = pd.to_numeric(df_i[dl_cn[0]], errors='coerce').fillna(0) / 1e6
            dl_nonzero = dl_vals[dl_vals > 0]
            data_dl_rates.extend(dl_nonzero.tolist())
            if operator not in data_operator_rates:
                data_operator_rates[operator] = {'dl_rates': [], 'ul_rates': []}
            data_operator_rates[operator]['dl_rates'].extend(dl_nonzero.tolist())
        if ul_cn:
            ul_vals = pd.to_numeric(df_i[ul_cn[0]], errors='coerce').fillna(0) / 1e6
            ul_nonzero = ul_vals[ul_vals > 0]
            data_ul_rates.extend(ul_nonzero.tolist())
            if operator not in data_operator_rates:
                data_operator_rates[operator] = {'dl_rates': [], 'ul_rates': []}
            data_operator_rates[operator]['ul_rates'].extend(ul_nonzero.tolist())
    ...
    log(f"数据文件合并完成: {len(df_data_raw)} 行, {len(data_dfs)} 个文件", 15)

    # 计算数据指标
    def calc_data_rates(rates, clip_val):
        if len(rates) == 0:
            return 0, 0
        clipped = [min(r, clip_val) for r in rates]
        mean_rate = round(np.mean(clipped), 2)
        sorted_rates = sorted(clipped, reverse=True)
        tn = max(1, int(len(sorted_rates) * 0.1))
        top10 = round(np.mean(sorted_rates[:tn]), 2)
        return top10, mean_rate

    data_dl_top10, data_dl_mean = calc_data_rates(data_dl_rates, 1000)
    data_ul_top10, data_ul_mean = calc_data_rates(data_ul_rates, 200)
    log(f"数据指标: 下行前10%峰值={data_dl_top10}Mbps, 下行均值={data_dl_mean}Mbps, "
        f"上行前10%峰值={data_ul_top10}Mbps, 上行均值={data_ul_mean}Mbps", 16)
    ...
    if len(df_msg) > 0 and 'MessageType' in df_msg.columns:
        ...
        while i < n:
            row = df_msg.iloc[i]
            msg_type = str(row.get('MessageType', ''))

            if msg_type == 'SIP_REQ_INVITE':
                # 取最后一个连续INVITE（时间差<1秒算连续）
                t1 = row['_t']
                t1_time = row['Time']
                inv_idx = i
                while inv_idx + 1 < n:
                    next_row = df_msg.iloc[inv_idx + 1]
                    next_msg = str(next_row.get('MessageType', ''))
                    if next_msg == 'SIP_REQ_INVITE' and (next_row['_t'] - df_msg.iloc[inv_idx]['_t']).total_seconds() < 1.0:
                        inv_idx = inv_idx + 1
                        t1 = df_msg.iloc[inv_idx]['_t']
                        t1_time = df_msg.iloc[inv_idx]['Time']
                    else:
                        break
                ...
                bye_time = None
                bye_time_raw = None
                last_msg_time = t1_time
                last_msg_time_raw = t1_time

                while j < n:
                    curr_row = df_msg.iloc[j]
                    curr_msg = str(curr_row.get('MessageType', ''))
                    curr_time = curr_row['_t']
                    curr_time_raw = curr_row['Time']

                    # 遇到下一个INVITE（与前一个INVITE时间差>=1秒），停止
                    if curr_msg == 'SIP_REQ_INVITE':
                        break

                    # 记录最后一条消息的时间
                    last_msg_time = curr_time
                    last_msg_time_raw = curr_time_raw

                    # 找第一个SIP_RSP_180
                    if not found_180 and curr_msg == 'SIP_RSP_180':
                        t2 = curr_time
                        found_180 = True

                    # 找SIP_REQ_BYE（标记已找到）
                    if curr_msg == 'SIP_REQ_BYE':
                        found_bye = True
                        bye_time = curr_time
                        bye_time_raw = curr_time_raw
                        # 在1秒内找SIP_RSP_200
                        k = j + 1
                        while k < n:
                            check_row = df_msg.iloc[k]
                            check_msg = str(check_row.get('MessageType', ''))
                            if check_msg == 'SIP_REQ_INVITE':
                                break
                            if check_msg == 'SIP_RSP_200' and (check_row['_t'] - bye_time).total_seconds() <= 1:
                                found_bye_200 = True
                                break
                            k += 1
                ...
                # 呼叫建立时长
                call_setup_duration = None
                if t2 is not None:
                    call_setup_duration = int((t2 - t1).total_seconds() * 1000)

                # 完整通话判定
                is_complete = '是' if found_bye_200 else '否'

                # 结束时间
                if found_bye_200 and bye_time_raw is not None:
                    end_raw = bye_time_raw
                else:
                    end_raw = last_msg_time_raw

                rnd += 1
                voice_records.append({
                    '轮次': rnd,
                    '运营商': _detect_operator(str(row.get('文件名', '')) + str(row.get('运营商', ''))),
                    '开始时间': t1_time,
                    '结束时间': end_raw,
                    '呼叫建立时长(s)': round(call_setup_duration / 1000, 2) if call_setup_duration is not None else '',
                    '是否完整通话': is_complete
                })
    ...
    for f in voice_files:
        ...
        # 计算该文件的通话轮次、平均呼叫建立时长、优质通话占比
        # 从voice_records中统计该文件涉及的轮次
        file_rounds = 0
        file_setup_vals = []
        for vr in voice_records:
            # 判断该轮次是否属于这个文件：通过开始时间/结束时间与文件数据时间范围对比
            # 简单方法：检查voice_records中的运营商与文件运营商一致，且该轮次范围在文件时间范围内
            pass  # 下面重新统计
        ...
        file_time_range_str = ''
        # 从voice_dfs获取行数和时间范围
        for df_i in voice_dfs:
            if f in df_i.get('_source_file', []):
                file_row_count = len(df_i)
                times = df_i['_t'].dropna()
                if len(times) > 0:
                    file_time_range_str = f"{times.min().strftime('%Y-%m-%d %H:%M:%S')} ~ {times.max().strftime('%Y-%m-%d %H:%M:%S')}"
                break

        # 如果语音文件有MessageType列，说明是语音文件，统计voice_records中属于该文件的部分
        # 按文件名匹配：找voice_records中开始时间落在该文件时间范围内的轮次
        file_voice_records = []
        # 先读取文件的时间范围
        f_start = None
        f_end = None
        if file_time_range_str:
            try:
                parts = file_time_range_str.split(' ~ ')
                if len(parts) == 2:
                    f_start = datetime.strptime(parts[0], '%Y-%m-%d %H:%M:%S')
                    f_end = datetime.strptime(parts[1], '%Y-%m-%d %H:%M:%S')
            except:
                pass

        # 如果该文件有语音数据，则统计voice_records
        if file_type == '语音' and f_start is not None:
            for vr in voice_records:
                try:
                    vr_time = datetime.strptime(str(vr.get('开始时间', '')).split('.')[0], '%Y-%m-%d %H:%M:%S') if vr.get('开始时间') else None
                except:
                    vr_time = None
                if vr_time and f_start and f_end and f_start <= vr_time <= f_end:
                    file_voice_records.append(vr)
        ...
        # 优质通话占比(MOS3.5)：从df_vqi中统计该文件的时间范围内的MOS值
        file_vqi_records = []
        if len(df_vqi) > 0 and f_start is not None:
            for _, vqi_row in df_vqi.iterrows():
                try:
                    vqi_time = datetime.strptime(str(vqi_row.get('时间', '')).split('.')[0], '%Y-%m-%d %H:%M:%S') if vqi_row.get('时间') else None
                except:
                    vqi_time = None
                if vqi_time and f_start and f_end and f_start <= vqi_time <= f_end:
                    file_vqi_records.append(vqi_row)
        file_quality_pct = ''
        if file_vqi_records:
            vqi_df = pd.DataFrame(file_vqi_records)
            total_file = len(vqi_df)
            quality_file = (vqi_df['是否优质(MOS>3.5)'] == '是').sum() if '是否优质(MOS>3.5)' in vqi_df.columns else 0
            if total_file > 0:
                file_quality_pct = f"{round(quality_file / total_file * 100, 2)}%"

        # 如果没有从voice_dfs获取到行数和时间范围，则重新读取
        if file_row_count == 0:
            try:
                df_tmp = _read_file(f)
                file_row_count = len(df_tmp)
                times = []
                if 'Time' in df_tmp.columns:
                    for t in df_tmp['Time']:
                        pt = parse_time(t)
                        if pt:
                            times.append(pt)
                if times:
                    file_time_range_str = f"{min(times).strftime('%Y-%m-%d %H:%M:%S')} ~ {max(times).strftime('%Y-%m-%d %H:%M:%S')}"
            except Exception:
                pass
    ...
    for f in data_files:
        ...
        try:
            ...
            if times:
                file_time_range_str = f"{min(times).strftime('%Y-%m-%d %H:%M:%S')} ~ {max(times).strftime('%Y-%m-%d %H:%M:%S')}"
            dl_cn = [c for c in df_tmp.columns if 'RLC' in c and 'Downlink' in c]
            ul_cn = [c for c in df_tmp.columns if 'RLC' in c and 'Uplink' in c]
            if dl_cn:
                dl_vals = pd.to_numeric(df_tmp[dl_cn[0]], errors='coerce').fillna(0) / 1e6
                dl_nonzero = dl_vals[dl_vals > 0]
                if len(dl_nonzero) > 0:
                    clipped = [min(r, 1000) for r in dl_nonzero]
                    file_dl_mean = round(np.mean(clipped), 2)
                    sorted_rates = sorted(clipped, reverse=True)
                    tn = max(1, int(len(sorted_rates) * 0.1))
                    file_dl_top10 = round(np.mean(sorted_rates[:tn]), 2)
            if ul_cn:
                ul_vals = pd.to_numeric(df_tmp[ul_cn[0]], errors='coerce').fillna(0) / 1e6
                ul_nonzero = ul_vals[ul_vals > 0]
                if len(ul_nonzero) > 0:
                    clipped = [min(r, 200) for r in ul_nonzero]
                    file_ul_mean = round(np.mean(clipped), 2)
                    sorted_rates = sorted(clipped, reverse=True)
                    tn = max(1, int(len(sorted_rates) * 0.1))
                    file_ul_top10 = round(np.mean(sorted_rates[:tn]), 2)
        except Exception:
            pass
        file_stats.append({
            '文件名': filename_base,
            ...
            '平均呼叫建立时长(s)': '',
            '优质通话占比(MOS3.5)': '',
            '下行前10%峰值速率': file_dl_top10,
            '下行均值速率': file_dl_mean,
            '上行前10%峰值速率': file_ul_top10,
            '上行均值速率': file_ul_mean
        })
    ...
    for op in ['电信', '联通']:
        ...
        op_ul_top10 = ''
        op_ul_mean = ''
        if op in data_operator_rates:
            op_dl = data_operator_rates[op].get('dl_rates', [])
            op_ul = data_operator_rates[op].get('ul_rates', [])
            if op_dl:
                clipped = [min(r, 1000) for r in op_dl]
                op_dl_mean = round(np.mean(clipped), 2)
                sorted_rates = sorted(clipped, reverse=True)
                tn = max(1, int(len(sorted_rates) * 0.1))
                op_dl_top10 = round(np.mean(sorted_rates[:tn]), 2)
            if op_ul:
                clipped = [min(r, 200) for r in op_ul]
                op_ul_mean = round(np.mean(clipped), 2)
                sorted_rates = sorted(clipped, reverse=True)
                tn = max(1, int(len(sorted_rates) * 0.1))
                op_ul_top10 = round(np.mean(sorted_rates[:tn]), 2)
        ...
        operator_stats.append({
            ...
            '下行前10%峰值速率': op_dl_top10,
            '下行均值速率': op_dl_mean,
            '上行前10%峰值速率': op_ul_top10,
            '上行均值速率': op_ul_mean,
            '微信大文件上传中位数时长(s)': '',
            '微信大文件发送平均速率(Mbps)': '',
            '微信小文件发送平均时延(s)': '',
    ...
    for op in ['电信', '联通']:
        ...
        summary_data.append({
            '运营商': op,
            '文件名': f"{len(df_file_stats[df_file_stats['运营商'] == op]) if len(df_file_stats) > 0 else 0}个文件" if len(df_file_stats) > 0 else '',
            '起止时间': time_range_str,
            '呼叫建立时长(s)': avg_setup,
            '优质通话占比(MOS3.5)': quality_pct_op,
            '下行前10%峰值速率': op_dl_top10,
    ...
    log("写入输出文件...", 70)
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        # 根据merge_raw决定输出哪些sheet
        if merge_raw:
            # 输出原始数据sheet
            if len(df_voice_raw) > 0:
                df_voice_raw_out = df_voice_raw.copy()
                if '_t' in df_voice_raw_out.columns:
                    df_voice_raw_out = df_voice_raw_out.drop('_t', axis=1)
                if '_time_only' in df_voice_raw_out.columns:
                    df_voice_raw_out = df_voice_raw_out.drop('_time_only', axis=1)
                df_voice_raw_out.to_excel(w, sheet_name='语音业务-原始文件', index=False)
            if len(df_data_raw) > 0:
                df_data_raw_out = df_data_raw.copy()
                if '_t' in df_data_raw_out.columns:
                    df_data_raw_out = df_data_raw_out.drop('_t', axis=1)
                df_data_raw_out.to_excel(w, sheet_name='数据业务-原始文件', index=False)
        ...
        # 格式化原始数据sheet（根据merge_raw）
        if merge_raw:
            if '语音业务-原始文件' in w.sheets:
                ...
                for col_idx in range(1, ws_raw.max_column + 1):
                    mx = len(str(ws_raw.cell(row=1, column=col_idx).value or ''))
                    for r in range(2, min(102, ws_raw.max_row + 1)):
                        cv = ws_raw.cell(row=r, column=col_idx).value
                        if cv is not None: mx = max(mx, min(len(str(cv)), 30))
                    ws_raw.column_dimensions[_gcl(col_idx)].width = max(mx + 2, 10)
                ws_raw.freeze_panes = 'A2'

            if '数据业务-原始文件' in w.sheets:
                ws_data_raw = w.sheets['数据业务-原始文件']
                for cell in ws_data_raw[1]:
                    cell.alignment = center; cell.fill = hdr_fill; cell.font = OpFont(bold=True); cell.border = border
                for col_idx in range(1, ws_data_raw.max_column + 1):
                    mx = len(str(ws_data_raw.cell(row=1, column=col_idx).value or ''))
                    ws_data_raw.column_dimensions[_gcl(col_idx)].width = max(mx + 2, 10)
                ws_data_raw.freeze_panes = 'A2'
            ...
            for ci in range(1, ws_voice.max_column + 1):
                ...
                if ci == 5:  # 呼叫建立时长(s)列
                    ...
                    if cv is None or str(cv) == '' or str(cv) == 'nan':
                        cell.comment = Comment("无RSP_180", "Tool")
...
if GUI_OK:
        ...
        def run(self):
            try:
                out = process_vqi(self.files,
                                  callback=self.l, cancel_check=lambda: self._cancel,
                                  progress_cb=lambda p: self.progress_pct.emit(int(p)),
                                  time_filter=self.time_filter, merge_raw=self.merge_raw)
                self.done.emit('' if self._cancel else out)
            except KeyboardInterrupt:
                self.done.emit('')
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.6")
        ...
        def vqi_run(self):
            ...
            merge_raw = self.vqi_cb_merge_raw.isChecked()
            self.vqi_thread = VqiWorkerThread(self.vqi_files, time_filter=time_filter, merge_raw=merge_raw)
        ...
        def show_vqi_result(self, path):
            try:
                ...
                self._fill_vqi_table(self.vqi_tbl_raw, wb['原始数据'], path=path, sheet='原始数据')
                ...
                self.vqi_lt.append("结果已内嵌显示（3个Sheet）")
            except Exception as e:
        ...
        def run(self):
            ...
            self.thread = WorkerThread(self.files, qci_list, self.sd.value(), self.su.value(),
                                       ...
                                       ftp_duration=ftp_duration, merge_raw=merge_raw)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.6 &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-18</b></p>"
                "<ul>"
                "<li><b>V2.5.6</b> 新增合并原始数据复选框（速率统计/VQI标签页），默认打勾</li>"
                "<li>新增批注辅助函数_add_calc_comment，在汇总-数据和语音/按文件名/按运营商三个sheet中为数值单元格加计算批注</li>"
                "<li>数据文件处理：process_vqi中数据文件（无MessageType列）不再跳过，读取并计算下行/上行速率指标</li>"
                "<li>输出Sheet完善：merge_raw=True时输出7个sheet（含语音业务-原始文件/数据业务-原始文件）</li>"
                "<li>性能优化：文件类型判断只读前3行（_read_file_head函数），缓存已读取的文件列表</li>"
                "<li>按文件名表数据文件行：通话轮次=0、平均呼叫建立时长=''、优质通话占比=''，填充数据速率指标</li>"
                "<li>按运营商表填充数据指标：下行前10%峰值速率、下行均值速率、上行前10%峰值速率、上行均值速率</li>"
