...
try:
    from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
        ...
        QTabWidget, QTableWidget, QTableWidgetItem, QHeaderView, QTimeEdit)
    from PySide6.QtCore import Qt, QThread, Signal, Slot, QTime
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None):
            super().__init__()
            self.files = files; self.qci_list = qci_list; self.dl_clip = dl_clip; self.ul_clip = ul_clip
            self.base_file = base_file; self.time_filter = time_filter
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.4.5")
        ...
        def _build(self):
            ...
            tabs.addTab(self._build_vqi_tab(), "5G虚拟用户跟踪用户语音VQI工具")
        ...
        def _build_rate_tab(self):
            ...
            self.result_tabs.addTab(self.tbl_compare, "对比横表")
        ...
        def _build_vqi_tab(self):
            ...
            lb = QLabel("5G虚拟用户跟踪用户语音VQI工具（界面布局同左侧，算法待接入）")
            lb.setFont(self._font(2)); lb.setAlignment(Qt.AlignCenter); lo.addWidget(lb)
            lo.addStretch(1); w.setFont(self._font()); return w

        def sf(self):
            fs, _ = QFileDialog.getOpenFileNames(self, "选择文件", "", "Excel/压缩 (*.xlsx *.xls *.zip);;All (*)")
            if fs:
                for f in fs:
        ...
        def show_result(self, path):
            try:
                ...
                self._fill_table(self.tbl_detail, wb['详细过程'], filter_a1=True, path=path, sheet='详细过程')
                self._fill_table(self.tbl_summary, wb['汇总'], filter_a1=False, path=path, sheet='汇总')
                self._fill_table(self.tbl_compare, wb['对比横表'], filter_a1=False, path=path, sheet='对比横表')
