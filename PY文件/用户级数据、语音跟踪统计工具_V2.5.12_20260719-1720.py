    ...
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        ...
        for col_letter, fill in [('D', fill_d), ('E', fill_e)]:
            for cell in ws[col_letter]:
                ...
                if cancel_check and cell.row % 5000 == 0 and cancel_check():
                    raise KeyboardInterrupt('用户取消')
        ...
        for ci in range(0, len(df_compare)):
            if cancel_check and ci % 20 == 0 and cancel_check():
                raise KeyboardInterrupt('用户取消')
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.12")
                ...
                if txt:
                    for line in txt.split('\n'):
                        ...
                        if os.path.isfile(line):
                            ...
                            self._parse_mapping_line(line, table)
                    # 重新读取表格(文本框内容已写入)
                    for row in range(table.rowCount()):
                        phone_item = table.item(row, 0)
                        tid_item = table.item(row, 1)
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("5G用户级公共监控速率统计工具 V2.5.12")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("5G虚拟用户跟踪语音VQI工具 V2.5.12")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            # 输入文件（选择/清空紧挨着，拖入框明显、高度翻倍）
            gf = QGroupBox("输入文件"); gfl = QVBoxLayout(gf)
            gfl.setContentsMargins(4, 4, 4, 4); gfl.setSpacing(2)
