    ...
    def _match_hybrid(self, agg, params):
        ...
        for biz, cfg in REF_BIZ.items():
            for r in ref[operator].get(biz, []):
                ...
                if biz == 'store_s':
                    # V1.12: 商店小包用dl_mac每秒max含零clip1000（诊断结论）
                    time_max = wdf.groupby('time')['dl_mac'].max()
                    if len(time_max) == 0:
                        continue
                    calc_rate = time_max.clip(upper=1000).mean()
                elif biz == 'store_l':
                    # V1.10: 商店大包用dl_rlc去零均值（去掉clip1000）
        ...
        for rd in round_results:
            ...
            wdf = pd.DataFrame(window)
            # 按秒聚合上行MAC
            per_sec = wdf.groupby('time')['ul_mac'].max()
            # 找连续的上行段（微信小包峰值>5Mbps，时长1~5秒，流量~5MB）
            times = list(per_sec.index)
            vals = list(per_sec.values)
            best_seg = None
            i = 0
            while i < len(times):
                if vals[i] > 5:  # 微信小包峰值>5Mbps
                    seg_start = i
                    ...
                    if len(seg_times) >= 1:
                        ...
                        # 微信小包固定5MB，容差±3MB（2~8MB）
                        if 2 <= flow_mb <= 8 and dur <= 5:
                            # 取peak作为速率（对齐微信小文件口径）
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('5G用户级公共监控速率统计工具 V1.13')
