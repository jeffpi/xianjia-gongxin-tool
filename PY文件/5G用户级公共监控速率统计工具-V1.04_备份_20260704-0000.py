...
class UCMProcessor:
    ...
    def _match_auto(self, agg, params, dl_min, ul_min):
        ...
        while i < n:
            if agg.at[i, 'dl_mac'] > dl_min or agg.at[i, 'ul_mac'] > ul_min:
                ...
                i += 1
        # 分类(用原始未扩展段,rate/flow/dur清晰)
        for idx, s in enumerate(segs):
            s['key'] = self._classify_seg(s, segs, idx)
        # 只对store_s扩展前后1秒含爬坡(对齐基准时延口径;其他业务不扩,靠_summarize全点均值)
        for s in segs:
        ...
        # 按业务key分组收集所有段(不限轮次:有多少段算多少,按业务汇总)
        segs_by_key = {}
        for s in segs:
            k = s.get('key')
            if k is None: continue
            # 空段过滤:点数<3或主方向流量<2MB的噪声段不纳入(对齐基准53业务段 vs 116空段)
    ...
    def _classify_seg(self, s, segs, idx):
        ...
        if d == '下行':
            if flow < 80: return None        # 极小流量→非业务
            if flow < 250: return 'store_s'   # 小流量→应用宝小包(基准113MB)
            if rate > 900: return 'store_l'   # 大流量+高速→应用宝大包(基准1026)
            if flow > 400: return 'ftp_dl'    # 大流量+中速→FTP下载(基准572)
            return None
        else:
            if flow < 5: return None         # 极小流量→非业务
            if dur < 6 and flow < 50: return 'wx_s'  # 短小→微信小包(基准1.5s,5MB)
            if dur > 10: return 'wx_l'       # 长时间→微信大文件(基准28s)
            return None
        return None
