...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None):
    ...
    for rnd_bizs in base_rounds:
        ...
        for k in range(n):
            ...
            # 由相邻业务边界确定匹配区间(非固定秒数窗口)
            lower = pd.to_datetime(rnd_bizs[k - 1]['结束时间']) if k > 0 else None
            upper = pd.to_datetime(rnd_bizs[k + 1]['开始时间']) if k < n - 1 else b_end

            cand = df_detail[(df_detail['businessType'] == b_biz) &
                             (df_detail['_t'] <= upper) &
                             (~df_detail.index.isin(used))]
            if lower is not None:
                cand = cand[cand['_t'] >= lower]

            if len(cand) > 0:
                # 区间内同业务行按时间排序，取首行=段开始、末行=段结束。
                # 不依赖行号连续(df_detail 存在隔行 NaN 标记)，避免连续段扩展失效。
                cand_s = cand.sort_values('_t')
                used.update(cand_s.index.tolist())
                first = cand_s.iloc[0]
                last = cand_s.iloc[-1]
    ...
    for item in all_items:
        ...
        if cd:
            ...
            cr.append({'轮次': rnd, '来源': '偏差', '业务类型': biz,
                '开始时间': '', '结束时间': '',
                '速率类指标': '速率偏差(%)', '数值': _pct(cd['数值'], bd['数值']),
                '时长类指标': '时长偏差(%)', '数值.1': _pct(cd['数值.1'], bd['数值.1'])})
        else:
            cr.append({'轮次': rnd, '来源': '代码', '业务类型': biz,
                '开始时间': '缺失', '结束时间': '缺失',
                '速率类指标': '缺失', '数值': '缺失',
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        ...
        # 按顺序写入：每3列一组（基准+代码+偏差），跟踪轮次边界用于合并
        col = 2
        color_toggle = 0
        prev_rnd = None
        rnd_start_col = 2  # 当前轮次的起始列
        keys = ['轮次','来源','业务类型','开始时间','结束时间',
                '速率类指标','数值','时长类指标','数值.1']
        fill_bias = PatternFill(start_color='FFF2CC', end_color='FFF2CC', fill_type='solid')

        for ci in range(0, len(df_compare), 3):
            if ci + 2 >= len(df_compare): break
            r1 = df_compare.iloc[ci]      # 基准
            r2 = df_compare.iloc[ci + 1]  # 代码
            r3 = df_compare.iloc[ci + 2]  # 偏差
            rnd = r1['轮次']
            ...
            for k, rr in enumerate([r1, r2, r3]):
                use_fill = fill_bias if k == 2 else fill
                for ri, key in enumerate(keys):
                    c = ws_h.cell(row=ri+1, column=col+k,
                                  value=rr[key] if pd.notna(rr.get(key)) else '')
                    c.fill = use_fill; c.alignment = center

            # 合并第3行（业务类型跨基准+代码+偏差）
            ws_h.merge_cells(start_row=3, start_column=col, end_row=3, end_column=col+2)
            col += 3
        ...
        # ===== 对比竖表（横表转置 + 配色/合并/冻结，便于竖向查看）=====
        from openpyxl.styles import Font as OpFont
        from openpyxl.utils import get_column_letter as _gcl
        ws_v = wb.create_sheet('对比竖表')
        v_headers = ['轮次', '来源', '业务类型', '开始时间', '结束时间',
                     '速率类指标', '数值', '时长类指标', '数值.1']
        head_fill = PatternFill(start_color='D9E1F2', end_color='D9E1F2', fill_type='solid')
        biz_c1 = PatternFill(start_color='DAEEF3', end_color='DAEEF3', fill_type='solid')
        biz_c2 = PatternFill(start_color='E2EFDA', end_color='E2EFDA', fill_type='solid')
        rnd_red = PatternFill(start_color='FFC7CE', end_color='FFC7CE', fill_type='solid')
        for ci, h in enumerate(v_headers, 1):
            c = ws_v.cell(row=1, column=ci, value=h)
            c.alignment = center; c.fill = head_fill; c.font = OpFont(bold=True)
        vr = 2; prev_rnd = None
        for hcol in range(2, ws_h.max_column + 1):
            vals = []
            for ri in range(1, 10):
                v = ws_h.cell(row=ri, column=hcol).value
                if v is None and ri in (1, 3):
                    for mr in ws_h.merged_cells.ranges:
                        if mr.min_row <= ri <= mr.max_row and mr.min_col <= hcol <= mr.max_col:
                            v = ws_h.cell(row=mr.min_row, column=mr.min_col).value; break
                vals.append(v if (v is not None and str(v) != '') else '')
            rnd_v, biz_v = str(vals[0]), str(vals[2])
            try: biz_fill = biz_c1 if BIZ_ORDER.index(biz_v) % 2 == 0 else biz_c2
            except Exception: biz_fill = biz_c1
            if rnd_v and rnd_v != prev_rnd: prev_rnd = rnd_v
            rnd_fill = rnd_red if ('不计' in rnd_v or '不完整' in rnd_v) else None
            for ri, v in enumerate(vals):
                c = ws_v.cell(row=vr, column=ri + 1, value=v); c.alignment = center
                c.fill = rnd_fill if (ri == 0 and rnd_fill) else biz_fill
            vr += 1
        # 同轮次连续行合并 第1列(轮次) 与 第3列(业务类型)
        for col_idx in (1, 3):
            ms = 2; pv = None
            for rr in range(2, vr):
                cv = ws_v.cell(row=rr, column=col_idx).value
                if cv != pv:
                    if ms < rr - 1:
                        ws_v.merge_cells(start_row=ms, start_column=col_idx, end_row=rr - 1, end_column=col_idx)
                        ws_v.cell(row=ms, column=col_idx).alignment = center
                    ms = rr; pv = cv
            if ms < vr - 1:
                ws_v.merge_cells(start_row=ms, start_column=col_idx, end_row=vr - 1, end_column=col_idx)
                ws_v.cell(row=ms, column=col_idx).alignment = center
        for cidx in range(1, 10):
            mx = 0
            for ridx in range(1, min(vr, 120)):
                cv = ws_v.cell(row=ridx, column=cidx).value
                if cv is not None: mx = max(mx, min(len(str(cv)), 28))
            ws_v.column_dimensions[_gcl(cidx)].width = max(mx + 2, 10)
        for ridx in range(1, vr):
            ws_v.row_dimensions[ridx].height = 18
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def l(self, msg):
            self._pct = min(self._pct + 25, 90)
            self.progress.emit(msg)
            self.progress_pct.emit(self._pct)

    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("5G用户级公共监控速率统计工具 V2.3.0")
        ...
        def dropEvent(self, event):
            new = [u.toLocalFile() for u in event.mimeData().urls()
                   if u.toLocalFile().lower().endswith(('.xlsx', '.xls', '.mmf'))]
            if new:
        ...
        def _build(self):
            ...
            title = QLabel("5G用户级公共监控速率统计工具  V2.3.0")
