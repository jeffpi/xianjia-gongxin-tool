...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None, biz_params=None, calc_ramp=False, calc_ramp_mid=False):
    ...
    def _match_strict(s, biz):
        ...
        if biz == '应用商店小文件下载':
            return dl > ul and STORE_S_FLOW_MIN <= dl <= STORE_S_FLOW_MAX and d <= 8
        if biz == '微信大包发送':
    ...
    for s in _big_sorted:
        ...
        if _match_strict(s, expected):
            if expected in ('应用商店小文件下载', '应用商店大文件下载'):
        ...
        if expected != first_biz and _match_strict(s, first_biz):
            if first_biz in ('应用商店小文件下载', '应用商店大文件下载'):
                _pos = 1
                s['biz'] = '未知'
                continue
    ...
    for s in fc:
       if id(s) not in paired:
           ...
           elif STORE_S_FLOW_MIN <= s['dl'] <= STORE_S_FLOW_MAX and s['dl'] > s['ul']: s['biz'] = '应用商店小文件下载'
           else: s['biz'] = fd(s)
    ...
    # 构建有序的(时间边界, 预期业务)表：多槽位轮转填充gap，供微信大包扫描等时间查表用
    ...
    for _i in range(len(_anchor_segs)):
        ...
        _next_biz = _anchor_segs[_i + 1][2] if _i + 1 < len(_anchor_segs) else None
        if _bi >= 0 and _next_start is not None and _et < _next_start and _next_biz:
            try: _nbi = _BIZ_ORDER.index(_next_biz)
            except ValueError: _nbi = _bi
            # 收集gap中所有需要轮转的BIZ_ORDER项（从_bi+1循环到_nbi-1）
            _gap_items = []
            _pos = (_bi + 1) % len(_BIZ_ORDER)
            while _pos != _nbi:
                _gap_items.append(_BIZ_ORDER[_pos])
                _pos = (_pos + 1) % len(_BIZ_ORDER)
            # 均匀分配gap时间段给各槽位
            if _gap_items:
                _gap_total = (_next_start - _et) / np.timedelta64(1, 's')
                _slot_sec = _gap_total / len(_gap_items)
                for _j, _gap_biz in enumerate(_gap_items):
                    _slot_st = _et + np.timedelta64(int(_j * _slot_sec), 's')
                    _slot_et = _et + np.timedelta64(int((_j + 1) * _slot_sec), 's')
                    if _slot_et > _slot_st:
                        _expect_list.append((_slot_st, _slot_et, _gap_biz))
    ...
    _store_new = []
    # ===== 商店小包/大包识别（顺序累积规则）=====
    # 规则：按轮次确定商店时间窗口（FTP对结束到下一个FTP对开始之间），
    #       窗口内DL活跃秒逐秒累加：≥900切小包→继续累加到≥9000确认大包→UL>5结束
    _all_dl_active = [(i, dl_v[i]) for i in range(len(ts_v)) if dl_v[i] > ul_v[i] and dl_v[i] > 4]
    if pi:
        for _ri, (_rnd, _pe, _ps) in enumerate(pi):
            # 商店窗口：本轮FTP对最后一个业务结束 到 下一轮FTP对开始（或数据末尾）
            _win_start = _pe  # 本轮FTP对结束时间
            _win_end = pi[_ri + 1][2] if _ri + 1 < len(pi) else ts_v[-1] + np.timedelta64(1, 's')
            # 窗口内所有DL活跃秒
            _win_dl = sorted([(i, dl_v[i]) for i in range(len(ts_v))
                             if _win_start < ts_v[i] < _win_end and dl_v[i] > ul_v[i] and dl_v[i] > 4],
                            key=lambda x: ts_v[x[0]])
            if not _win_dl: continue
            # 顺序累积：0=小包累加 1=大包累加
            _phase = 0; _cum = 0; _small_start = None; _large_start = None
            for _ptr, (_idx, _val) in enumerate(_win_dl):
                _cum += _val
                if _phase == 0:
                    if _small_start is None: _small_start = _ptr
                    if _cum >= STORE_S_FLOW_MIN:
                        _st = ts_v[_win_dl[_small_start][0]]; _et = ts_v[_idx]
                        _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                        _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                            'dl': _cum, 'ul': 0, '业务': '应用商店小文件下载', '有效': True, 'rnd': _rnd})
                        _cum = 0; _phase = 1; _large_start = None
                else:
                    if _large_start is None: _large_start = _ptr
                    if _idx + 1 < len(ul_v) and ul_v[_idx + 1] > 5:
                        if _cum >= STORE_L_FLOW_MIN:
                            _st = ts_v[_win_dl[_large_start][0]]; _et = ts_v[_idx]
                            _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                            _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                                'dl': _cum, 'ul': 0, '业务': '应用商店大文件下载', '有效': True, 'rnd': _rnd})
                        _cum = 0; _phase = 0; _small_start = None; _large_start = None
            # 窗口末尾：大包阶段末尾
            if _phase == 1 and _large_start is not None and len(_win_dl) > _large_start:
                if _cum >= STORE_L_FLOW_MIN:
                    _st = ts_v[_win_dl[_large_start][0]]; _et = ts_v[_win_dl[-1][0]]
                    _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                    _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                        'dl': _cum, 'ul': 0, '业务': '应用商店大文件下载', '有效': True, 'rnd': _rnd})
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("工信部数据、语音跟踪统计工具 V2.5.39_CX")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("工信部数据统计工具 V2.5.39_CX")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("工信部语音VQI工具 V2.5.39_CX")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            vs = QSplitter(Qt.Vertical); vs.setChildrenCollapsible(False)
            if sys.platform == 'win32':
                vs.setStyleSheet("QSplitter::handle:vertical { background: #c8c8c8; height: 3px; margin: 1px 0; }")
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.39_CX &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
