...
class UCMProcessor:
    ...
    def _match_hybrid(self, agg, params):
        ...
        REF_BIZ = {
            ...
            'store_l': {'dir': 'dl', 'col': 'dl_rlc',  'clip': None, 'drop_zero': True},  # V1.10: 去掉clip1000
        ...
        for biz, cfg in REF_BIZ.items():
            for r in ref[operator].get(biz, []):
                ...
                if biz == 'store_s':
                    # V1.14: 商店小包用dl_rlc每秒mean含零（偏差+5%，优于max的+31%和mac的-3%）
        ...
        # ===== V1.11: wx_s用wx_l参考时间戳精确定位（微信小包→大包无间隔，5MB上行）=====
        # 从raw_mmf里找每个wx_l开始时间前30秒内的5MB上行段作为wx_s
        import datetime as _dt
        wx_s_added = 0
        for rd in round_results:
            if 'wx_s' in rd or 'wx_l' not in rd:
                continue
            wx_l_start = rd['wx_l']['start_t']
            # 在wx_l开始前0~30秒找上行段（微信小包5MB，时长1~3秒）
            search_start = wx_l_start - _dt.timedelta(seconds=30)
            window = [x for x in raw_mmf if search_start <= x['time'] < wx_l_start]
            if not window:
                continue
            wdf = pd.DataFrame(window)
            # V1.14: 用ul_rlc按秒聚合（peak偏差+3.7%，优于ul_mac的+12.5%）
            per_sec = wdf.groupby('time')['ul_rlc'].max()
            # 找连续的上行段（微信小包峰值>3Mbps，时长1~5秒，流量~5MB）
            times = list(per_sec.index)
            vals = list(per_sec.values)
            best_seg = None
            i = 0
            while i < len(times):
                if vals[i] > 3:  # V1.14: 阈值5→3，更敏感
                    seg_start = i
                    while i < len(times) and vals[i] > 1:
                        i += 1
                    seg_end = i
                    seg_times = times[seg_start:seg_end]
                    seg_vals = vals[seg_start:seg_end]
                    if len(seg_times) >= 1:
                        flow_mb = sum(seg_vals) / 8
                        dur = (seg_times[-1] - seg_times[0]).total_seconds() + 1
                        # V1.14: 流量范围放宽到1~15MB，覆盖更多场景
                        if 1 <= flow_mb <= 15 and dur <= 5:
                            # 取ul_rlc的peak作为速率（对齐基准）
                            peak_rate = max(seg_vals)
                            # 覆盖points
                            mask = (agg['t'] >= seg_times[0]) & (agg['t'] <= seg_times[-1])
                            seg_df = agg[mask].copy()
                            rd['wx_s'] = {
                                'start_t': seg_times[0], 'end_t': seg_times[-1],
                                'start_i': 0, 'end_i': 0,
                                'rate': round(float(peak_rate), 3),
                                'clip_rate': round(float(peak_rate), 3),
                                'duration': dur, 'points': seg_df, 'result': '成功'
                            }
                            wx_s_added += 1
                            break
                i += 1

        # 兜底：auto段检测的wx_s补充（如果参考时间戳没找到）
        for rd in round_results:
            if 'wx_s' not in rd:
                for seg in biz_auto.get('wx_s', []):
                    seg_t = seg['start_t']
                    # 找该rd的时间范围
                    rd_start = min(rd[k]['start_t'] for k in rd)
                    rd_end = max(rd[k]['end_t'] for k in rd)
                    if rd_start <= seg_t <= rd_end + _dt.timedelta(seconds=120):
                        rd['wx_s'] = seg
                        break

        print(f"混合模式: 共{len(round_results)}轮 "
              f"(store_l={sum(1 for r in round_results if 'store_l' in r)} "
              f"wx_s={sum(1 for r in round_results if 'wx_s' in r)}, wx_s参考时间戳定位={wx_s_added})")
    ...
    def _classify_seg(self, s, segs, idx):
        ...
        上行分类树（V1.10修正微信小文件阈值）：
          flow<1 → None(噪声)
          1~20MB → wx_s（微信小包~5MB/1~3s，基准范围5.03~5.27MB）
        ...
        d = s['direction']; flow = s['flow_mb']; dur = s['duration']; rate = s['rate']
        if d == '下行':
            if flow <= 20:  # V1.10: 50→20，保留低速率FTP
                return None
            if 20 < flow < 350:
                return 'store_s'
            if 350 <= flow < 600:
            ...
            if flow < 1:  # V1.10: 3→1，避免微信小文件(~5MB)漏识别
                return None
            if flow < 20:
                return 'wx_s'
            if flow <= 80:
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('5G用户级公共监控速率统计工具 V1.14')
