...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    # 预提取为numpy数组(避免183次循环每次访问pandas列的开销,提速10-100倍)
    _t_np = df['_t'].values
    _qci_np = df['QCI'].isin(qci_list).values
    _biz_np = df['业务识别'].notna().values
    ...
    for s in valid:
        ...
        if lb:
            ...
            # numpy数组布尔运算(C级别,比pandas Series快10-100倍)
            mm2 = (_t_np >= ms2) & (_t_np <= me2) & _qci_np & _biz_np
            labels[mm2] = lb
    df['轮次'] = labels
    ...
    _out_tmp = out + '.tmp'
    with xlsxwriter.Workbook(_out_tmp, {'strings_to_numbers': False, 'nan_inf_to_errors': False, 'default_date_format': 'yyyy-mm-dd hh:mm:ss'}) as wb:
        # ===== 预定义 formats =====
        ...
        for i in range(n_detail):
            if cancel_check and i > 0 and i % 1000 == 0 and cancel_check():
                raise KeyboardInterrupt('用户取消')
            row0 = i + 1
            d_orig = _write_arrays[col_3_idx][i] if n_col_detail > col_3_idx else None
            e_orig = _write_arrays[col_4_idx][i] if n_col_detail > col_4_idx else None
            d_dash = _is_eo(d_orig)
            e_dash = _is_eo(e_orig)
            has_fi = any(_ne(_write_arrays[c][i]) for c in range(5, min(9, n_col_detail)))
            # 预收集所有列值(只对特殊列做替换, 同时转换numpy类型为Python原生类型)
            row_vals = []
            for ci in range(n_col_detail):
                v = _write_arrays[ci][i]
                if ci == 0: v = 0 if d_dash else v
                elif ci == col_3_idx: v = '-' if d_dash else v
                elif ci == col_4_idx: v = '-' if e_dash else v
                # 转换numpy类型为Python原生类型(xlsxwriter不支持numpy类型)
                if v is None or (isinstance(v, float) and pd.isna(v)):
                    ...
                    v = str(v)
                row_vals.append(v)
            # write_row批量写入整行(1次调用替代~106次逐格写,提速30-100倍)
            ws.write_row(row0, 0, row_vals)
            # 对下载/上传速率列单独覆盖写入(带数字格式)
            if not d_dash and n_col_detail > col_3_idx:
                fmt = fmt_d if has_fi else fmt_num
                try: ws.write_number(row0, col_3_idx, float(d_orig), fmt)
                except (ValueError, TypeError): pass
            if not e_dash and n_col_detail > col_4_idx:
                fmt = fmt_e if has_fi else fmt_num
                try: ws.write_number(row0, col_4_idx, float(e_orig), fmt)
                except (ValueError, TypeError): pass
...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    with pd.ExcelWriter(out, engine='xlsxwriter') as w:
        ...
        if merge_raw and len(df_raw_out) > 0:
            ...
            # 用xlsxwriter直接API(支持>104万行, pd.to_excel有Excel行数限制)
            ws_raw = workbook.add_worksheet('语音业务-原始文件')
            raw_cols = list(df_raw_out.columns)
            n_raw_cols = len(raw_cols)
            # 写表头
            for ci, col_name in enumerate(raw_cols):
                ws_raw.write(0, ci, str(col_name), hdr_fmt)
            # 写数据: numpy数组 + write_row整行写入
            log(f"  写入{len(df_raw_out)}行x{n_raw_cols}列...", 77)
            raw_data = df_raw_out.values
            for ri in range(len(df_raw_out)):
                ...
                for ci in range(n_raw_cols):
                    ...
                    if v is None or (isinstance(v, float) and pd.isna(v)):
                        row_vals.append('')
                    elif isinstance(v, (np.integer,)): row_vals.append(int(v))
                    elif isinstance(v, (np.floating,)): row_vals.append(float(v))
                    elif isinstance(v, (np.datetime64,)): row_vals.append(pd.Timestamp(v).to_pydatetime())
                    elif isinstance(v, (np.bool_,)): row_vals.append(bool(v))
                    elif isinstance(v, (np.str_, np.bytes_)): row_vals.append(str(v))
                    else: row_vals.append(str(v) if v is not None else '')
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.16")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("5G用户级公共监控速率统计工具 V2.5.16")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("5G虚拟用户跟踪语音VQI工具 V2.5.16")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            # 输入文件（选择/清空紧挨着，拖入框明显、高度翻倍）
            gf = QGroupBox("输入文件"); gfl = QVBoxLayout(gf)
            gfl.setContentsMargins(4, 4, 4, 4); gfl.setSpacing(2)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.16 &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-19</b></p>"
                "<ul>"
                "<li><b>V2.5.16</b> 完整通话规则改为只看BYE(不再找200);RSRP拆分为(全部)和(通话时)两列;批注增加每轮次明细;汇总表/按文件名表列顺序调整(新增主被叫/手机号/类型);删除运营商表;新增12个数据明细sheet(语音6+数据6);运营商检测增加QCI兜底(路径无法判断时按QCI:7→电信/5或6→联通);默认值调整(QCI6+7都勾选,合并原始数据默认开,添加批注默认开);数据业务写入从openpyxl改为xlsxwriter逐行写(提速31%,可中断);手机号列构建性能优化(解决大量文件时卡顿);临时文件安全写入(取消不留坏文件)</li>\n"
