...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20):
...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None):
    ...
    log(f"  语音文件: {len(voice_files)} 个, 跳过的数据文件: {data_files_skipped} 个", 8)
    ...
    for op in ['电信', '联通']:
        ...
        summary_data.append({
            ...
            '呼叫建立时长(s)': avg_setup,
            '优质通话占比(MOS3.5)': quality_pct_op,
            '下行前10%峰值速率': '',  # 数据业务指标需从process函数输出中读取
            '下行均值速率': '',
            '上行前10%峰值速率': '',
            '上行均值速率': '',
            '微信大文件上传中位数时长(s)': '',
    ...
    log("写入输出文件...", 70)
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        df_raw_out.to_excel(w, sheet_name='原始数据', index=False)
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None, ftp_duration=20):
            super().__init__()
        ...
        def run(self):
            try:
                out = process(self.files, self.qci_list, self.dl_clip, self.ul_clip,
                              ...
                              ftp_duration=self.ftp_duration)
    ...
    class VqiWorkerThread(QThread):
        ...
        def __init__(self, files, time_filter=None):
            super().__init__()
            self.files = files
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.5")
        ...
        def vqi_run(self):
            ...
            self.vqi_thread = VqiWorkerThread(self.vqi_files, time_filter=time_filter)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.5 &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-18</b></p>"
                "<ul>"
                "<li><b>V2.5.5</b> 修复文件类型显示为unknown问题：_detect_file_type返回"语音"/"数据"而非"unknown"</li>"
                "<li>按文件名表新增指标列：通话轮次、平均呼叫建立时长(s)、优质通话占比(MOS3.5)</li>"
                "<li>按运营商表新增完整指标列：下行前10%峰值速率、下行均值速率、上行前10%峰值速率、上行均值速率、微信大文件上传中位数时长(s)、微信大文件发送平均速率(Mbps)、微信小文件发送平均时延(s)、微信小文件发送平均速率(Mbps)、商店大app平均时延(s)、商店大app平均速率(Mbps)、商店小app平均时延(s)、商店小app平均速率(Mbps)（当前留空）</li>"
                "<li>语音指标统计sheet中对空呼叫建立时长单元格加批注"无RSP_180"</li>"
