...
FTP_PAIR_GAP = 90.0
# 求和量区间（优先于时长判定，适应DT场景）
STORE_S_FLOW_MIN, STORE_S_FLOW_MAX = 900, 1500      # 应用商店小文件 DL求和量
STORE_L_FLOW_MIN, STORE_L_FLOW_MAX = 9000, 12000    # 应用商店大文件 DL求和量
WX_S_FLOW_MIN, WX_S_FLOW_MAX = 38, 50               # 微信小包 UL求和量
WX_L_FLOW_MIN, WX_L_FLOW_MAX = 1500, 2500           # 微信大包 UL求和量
# 宽松区间
STORE_S_FLOW_LOOSE = (500, 2000)
STORE_L_FLOW_LOOSE = (5000, 20000)
WX_S_FLOW_LOOSE = (20, 80)
WX_L_FLOW_LOOSE = (1000, 3000)
...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None, biz_params=None, calc_ramp=False, calc_ramp_mid=False):
    ...
    def _match_strict(s, biz):
        ...
        if biz == '应用商店大文件下载':
            return dl > ul and STORE_L_FLOW_MIN <= dl <= STORE_L_FLOW_MAX
        if biz == '应用商店小文件下载':
            return dl > ul and STORE_S_FLOW_MIN <= dl <= STORE_S_FLOW_MAX and d <= 5
        if biz == '微信大包发送':
            return ul > dl and WX_L_FLOW_MIN <= ul <= WX_L_FLOW_MAX and d >= 10
        if biz == '微信小包发送':
            return ul > dl and WX_S_FLOW_MIN <= ul <= WX_S_FLOW_MAX and d <= 5
        return False
    ...
    def _match_relaxed(s, biz):
        ...
        if biz == '应用商店大文件下载':
            return dl > ul and STORE_L_FLOW_LOOSE[0] <= dl <= STORE_L_FLOW_LOOSE[1]
        if biz == '应用商店小文件下载':
            return dl > ul and STORE_S_FLOW_LOOSE[0] <= dl <= STORE_S_FLOW_LOOSE[1] and d <= 10
        if biz == '微信大包发送':
            return ul > dl and WX_L_FLOW_LOOSE[0] <= ul <= WX_L_FLOW_LOOSE[1] and d >= 5
        if biz == '微信小包发送':
            return ul > dl and WX_S_FLOW_LOOSE[0] <= ul <= WX_S_FLOW_LOOSE[1] and d <= 10
        return False
    ...
    for s in _big_sorted:
        ...
        _found = False
        _big_bizs = {'FTP下载', 'FTP上传', '微信大包发送', '应用商店大文件下载'}
    ...
    for s in fc:
        if id(s) not in paired:
            if STORE_L_FLOW_MIN <= s['dl'] <= STORE_L_FLOW_MAX and s['dl'] > s['ul']: s['biz'] = '应用商店大文件下载'
            else: s['biz'] = fd(s)
    ...
    for idx, (rnd, pe, ps) in enumerate(pi):
        ...
        for bs in bw:
            ...
            if st2 and prev < bs['st'] and (bs['st'] - prev) / np.timedelta64(1, 's') > 1:
                ...
                if st2 == '应用商店小文件下载':
                    dls = [s for s in vd if s['dl'] > s['ul'] and STORE_S_FLOW_LOOSE[0] <= s['dl'] <= STORE_S_FLOW_LOOSE[1]]
                    if dls: bm = max(dls, key=lambda x: x['dl']); bm['业务'] = st2; bm['rnd'] = rnd; small.append(bm)
                else:
                    # 微信小包：反向扫描找离微信大包最近的UL短突发
                    uls = [s for s in vd if s['ul'] > s['dl'] and WX_S_FLOW_LOOSE[0] <= s['ul'] <= WX_S_FLOW_LOOSE[1]]
                    if uls: bm = max(uls, key=lambda x: x['st']); bm['业务'] = st2; bm['rnd'] = rnd; small.append(bm)
            prev = bs['et']
    ...
    if pi:
        ...
        if first_ftp_start > data_start + np.timedelta64(30, 's'):
                ...
                for bs in pre_big:
                    ...
                    if st2 and (bs['st'] - rng_start) / np.timedelta64(1, 's') > 1:
                        ...
                        if st2 == '应用商店小文件下载':
                            ...
                            uls = [s for s in vd if s['ul'] > s['dl'] and WX_S_FLOW_LOOSE[0] <= s['ul'] <= WX_S_FLOW_LOOSE[1]]
                            if uls: bm = max(uls, key=lambda x: x['st']); bm['业务'] = st2; bm['rnd'] = 0; small.append(bm)
                    rng_start = bs['et']
            ...
            if first_ftp_start > rng_start + np.timedelta64(1, 's'):
                ...
                uls = [s for s in vd if s['ul'] > s['dl'] and s['dur'] >= 0.5 and s['ul'] >= 20]
                if uls: bm = max(uls, key=lambda x: x['st']); bm['业务'] = '微信小包发送'; bm['rnd'] = 0; small.append(bm)
    ...
    # 第一个FTP对之前的段全部标为未知（rnd=0/None或st早于首业务）
    _first_biz_st = None
    for s in sorted(af, key=lambda x: x['st']):
        if s.get('rnd') not in (None, 0) and s.get('业务') not in (None, '未知', ''):
            _first_biz_st = s['st']
            break
    if _first_biz_st is not None:
        for s in af:
            if s.get('rnd') == 0 or (s['st'] < _first_biz_st - np.timedelta64(1, 's')):
                s['业务'] = '未知'
                s['有效'] = False
                s['rnd'] = None

    # ===== 商店/微信大包窗口扫描：查BIZ_ORDER顺序表，只有"轮到"对应业务才标 =====
    # 1) 构建时间→预期业务顺序表：已识别锚点按时间排序，相邻锚点间按BIZ_ORDER轮转推导空位预期业务
    _BIZ_ORDER = BIZ_ORDER
    # 收集状态机已识别的有效锚点业务段（去掉-爬坡/-掉速后缀；排除应用商店/微信大包本身——它们由扫描补全）
    _anchor_segs = []
    for s in af:
        if not s.get('有效') or not s.get('业务'): continue
        _b = str(s['业务']).split('-')[0]
        if _b in ('未知', '') or _b.startswith('应用商店') or _b == '微信大包发送': continue
        if _b in _BIZ_ORDER:
            _anchor_segs.append((s['st'], s['et'], _b))
    _anchor_segs.sort(key=lambda x: x[0])
    # 构建有序的(时间边界, 预期业务)表：每个锚点结束后的空位，预期业务=该锚点在BIZ_ORDER的下一项
    _expect_list = []  # [(t_start, t_end, expected_biz)]
    for _i in range(len(_anchor_segs)):
        _st, _et, _b = _anchor_segs[_i]
        try: _bi = _BIZ_ORDER.index(_b)
        except ValueError: _bi = -1
        # 当前锚点时段：预期=自身
        _expect_list.append((_st, _et, _b))
        # 锚点结束→下一锚点开始之间：逐项轮转推导
        _next_start = _anchor_segs[_i + 1][0] if _i + 1 < len(_anchor_segs) else None
        if _bi >= 0 and _next_start is not None and _et < _next_start:
            _slot_biz = _BIZ_ORDER[(_bi + 1) % len(_BIZ_ORDER)]
            _expect_list.append((_et, _next_start, _slot_biz))
    def _expected_biz(_t):
        """查顺序表，返回时间t按BIZ_ORDER轮转预期轮到的业务；查不到返回None"""
        for _st, _et, _b in _expect_list:
            if _st <= _t <= _et: return _b
        return None

    def _split_bursts(active_list):
        """活跃秒列表按连续性分段（gap>5s断开）"""
        if not active_list: return []
        bursts = []; cur = [active_list[0]]
        for _i in range(1, len(active_list)):
            _gap = (ts_v[active_list[_i][0]] - ts_v[active_list[_i-1][0]]) / np.timedelta64(1, 's')
            if _gap <= 5: cur.append(active_list[_i])
            else: bursts.append(cur); cur = [active_list[_i]]
        bursts.append(cur)
        return bursts

    _store_new = []
    # 2a) 应用商店扫描：DL活跃秒(dl>ul且dl>4)分段，burst中点查表，预期轮到应用商店才标
    _all_dl_active = [(i, dl_v[i]) for i in range(len(ts_v)) if dl_v[i] > ul_v[i] and dl_v[i] > 4]
    for _burst in _split_bursts(_all_dl_active):
        _mid_t = ts_v[_burst[len(_burst)//2][0]]
        _exp = _expected_biz(_mid_t)
        if _exp not in ('应用商店小文件下载', '应用商店大文件下载'): continue
        _cum = 0; _small_cut = -1
        for _vi, (_idx, _val) in enumerate(_burst):
            _cum += _val
            if _cum >= STORE_S_FLOW_MIN:
                _st = ts_v[_burst[0][0]]; _et = ts_v[_idx]
                _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                    'dl': _cum, 'ul': 0, '业务': '应用商店小文件下载', '有效': True, 'rnd': None})
                _small_cut = _vi + 1
                break
        if 0 <= _small_cut < len(_burst):
            _cum = sum(_burst[_j][1] for _j in range(_small_cut, len(_burst)))
            if _cum >= STORE_L_FLOW_MIN:
                _st = ts_v[_burst[_small_cut][0]]; _et = ts_v[_burst[-1][0]]
                _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                    'dl': _cum, 'ul': 0, '业务': '应用商店大文件下载', '有效': True, 'rnd': None})
        elif _small_cut < 0:
            _cum = sum(_d[1] for _d in _burst)
            if _cum >= STORE_L_FLOW_MIN:
                _st = ts_v[_burst[0][0]]; _et = ts_v[_burst[-1][0]]
                _dur = (_et - _st) / np.timedelta64(1, 's') + 1
                _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                    'dl': _cum, 'ul': 0, '业务': '应用商店大文件下载', '有效': True, 'rnd': None})
    # 2b) 微信大包扫描：UL活跃秒(ul>dl且ul>4)分段，burst中点查表，预期轮到微信大包且累计UL≥1650才标
    _all_ul_active = [(i, ul_v[i]) for i in range(len(ts_v)) if ul_v[i] > dl_v[i] and ul_v[i] > 4]
    for _burst in _split_bursts(_all_ul_active):
        _mid_t = ts_v[_burst[len(_burst)//2][0]]
        _exp = _expected_biz(_mid_t)
        if _exp != '微信大包发送': continue
        _cum = sum(_d[1] for _d in _burst)
        if _cum >= WX_L_FLOW_MIN:  # 1650
            _st = ts_v[_burst[0][0]]; _et = ts_v[_burst[-1][0]]
            _dur = (_et - _st) / np.timedelta64(1, 's') + 1
            _store_new.append({'st': _st, 'et': _et, 'dur': _dur,
                'dl': 0, 'ul': _cum, '业务': '微信大包发送', '有效': True, 'rnd': None})
    af.extend(_store_new)
    ...
    for si, seg in enumerate(valid):
        ...
        # 所有业务：只标记有流量的行
        # FTP大业务用10Mbps阈值(去掉头尾零散小值), 其它用>0
        cv_all = df.loc[sm, col].fillna(0)
        _ftp_th = 10.0 if biz in ('FTP下载', 'FTP上传') else 0.0
        nz = cv_all > _ftp_th
        ...
        if biz in ('FTP下载', 'FTP上传'):
            ...
            # 重新计算标记范围（绝对阈值10Mbps → 相对阈值峰值15%二次裁剪）
            _ftp_end_th = 10.0 if biz in ('FTP下载', 'FTP上传') else 0.0
            mm_final = (df['_t'] >= first_t) & (df['_t'] <= last_t) & (df['QCI'].isin(qci_list)) & (df[col].fillna(0) > _ftp_end_th)
            # FTP相对阈值裁剪：掐掉首尾低于峰值15%的值
            if biz in ('FTP下载', 'FTP上传') and mm_final.any():
                _seg_vals = df.loc[mm_final, col].fillna(0)
                _peak = _seg_vals.max()
                if _peak > 0:
                    _rel_th = 10.0  # 尾部保留到10Mbps, 低于10才截断
                    _rel_mask = _seg_vals >= _rel_th
                    if _rel_mask.any():
                        _rel_idx = _seg_vals.index[_rel_mask]
                        mm_final = mm_final & (df.index >= _rel_idx[0]) & (df.index <= _rel_idx[-1])
                        # 25%阈值裁剪后更新首尾时间和时长
                        _new_times = df.loc[mm_final, '_t']
                        if len(_new_times) > 0:
                            first_t, last_t = _new_times.min(), _new_times.max()
            md = (last_t - first_t).total_seconds()
            ...
            # FTP爬坡/掉速标记：速率<峰值10%的行 → 根据位置加-爬坡或-掉速后缀
            _ftp_peak = df.loc[mm_final, col].max()
            if _ftp_peak > 0:
                _ftp_full = sm & (df[col].fillna(0) > 0) & ~mm_final
                if _ftp_full.any():
                    _ftp_low = _ftp_full & (df[col].fillna(0) < _ftp_peak * 0.1)
                    _ftp_ramp = _ftp_low & (((df['_t'] < first_t) & (df['_t'] >= first_t - timedelta(seconds=10))) | ((df['_t'] > last_t) & (df['_t'] <= last_t + timedelta(seconds=10))))
                    _ftp_dip = _ftp_low & ~_ftp_ramp
                    _ftp_normal = _ftp_full & ~_ftp_low
                    if _ftp_ramp.any():
                        df.loc[_ftp_ramp, '业务识别'] = biz + '-爬坡'
                        df.loc[_ftp_ramp, '持续时长'] = round(md, 3)
                        df.loc[_ftp_ramp, '速率(Mbps)'] = round(ar, 2)
                    if _ftp_dip.any():
                        df.loc[_ftp_dip, '业务识别'] = biz + '-掉速'
                        df.loc[_ftp_dip, '持续时长'] = round(md, 3)
                        df.loc[_ftp_dip, '速率(Mbps)'] = round(ar, 2)
                    if _ftp_normal.any():
                        df.loc[_ftp_normal, '业务识别'] = biz
                        df.loc[_ftp_normal, '持续时长'] = round(md, 3)
                        df.loc[_ftp_normal, '速率(Mbps)'] = round(ar, 2)
            continue

        # 其他业务：全量标记有流量的行，按峰值10%分核心/爬坡/掉速
        _nf_full = sm & (df[col].fillna(0) > 0)
        if not _nf_full.any():
            continue
        _nf_vals = df.loc[_nf_full, col].fillna(0)
        _nf_peak = _nf_vals.max()
        _nf_th = _nf_peak * 0.1 if _nf_peak > 0 else 10.0
        _nf_core_mask = _nf_full & (df[col].fillna(0) >= _nf_th)
        _nf_ramp_mask = _nf_full & ~_nf_core_mask

        first_t = df.loc[_nf_full, '_t'].min()
        last_t = df.loc[_nf_full, '_t'].max()
        _core_first = df.loc[_nf_core_mask, '_t'].min() if _nf_core_mask.any() else first_t
        _core_last = df.loc[_nf_core_mask, '_t'].max() if _nf_core_mask.any() else last_t
        md = (last_t - first_t).total_seconds()

        # 核心行
        if _nf_core_mask.any():
            df.loc[_nf_core_mask, '业务识别'] = biz
            df.loc[_nf_core_mask, '持续时长'] = round(md, 3)
        # 爬坡/掉速行
        if _nf_ramp_mask.any():
            _nf_ramp_edge = _nf_ramp_mask & (((df['_t'] < _core_first) & (df['_t'] >= _core_first - timedelta(seconds=10))) | ((df['_t'] > _core_last) & (df['_t'] <= _core_last + timedelta(seconds=10))))
            _nf_ramp_mid = _nf_ramp_mask & ~_nf_ramp_edge
            if _nf_ramp_edge.any():
                df.loc[_nf_ramp_edge, '业务识别'] = biz + '-爬坡'
                df.loc[_nf_ramp_edge, '持续时长'] = round(md, 3)
            if _nf_ramp_mid.any():
                df.loc[_nf_ramp_mid, '业务识别'] = biz + '-掉速'
                df.loc[_nf_ramp_mid, '持续时长'] = round(md, 3)
        # 速率
        if biz in DL_BIZ: vs = pd.to_numeric(df.loc[_nf_full, '下载速率'], errors='coerce').fillna(0).tolist()
        else: vs = pd.to_numeric(df.loc[_nf_full, '上传速率'], errors='coerce').fillna(0).tolist()
        ar = sum(vs) / md if md > 0 else 0
        df.loc[_nf_full, '速率(Mbps)'] = round(ar, 2)
        seg['mark_st'], seg['mark_et'], seg['mark_dur'] = _core_first, _core_last, md
        # 用核心边界替代全段边界（爬坡只从核心往前算，不包含遥远孤立点）
        first_t, last_t = _core_first, _core_last

        # 爬坡速率：扩展段边界覆盖全部st~et范围（任一勾选后生效）
        # 爬坡/掉速标记始终执行（对勾只控制汇总是否纳入计算）
        _active_core = mm_final if 'mm_final' in locals() else mm
        _ramp_sm = (df['_t'] >= seg['st']) & (df['_t'] <= seg['et'] + timedelta(seconds=1)) & (df['QCI'].isin(qci_list))
        _ramp_col = '_dl' if biz in DL_BIZ else '_ul'
        _ramp_nz = df.loc[_ramp_sm, _ramp_col].fillna(0) > 0
        _ramp_full = _ramp_sm & _ramp_nz.reindex(df.index, fill_value=False)
        # 全模式标记：1=首部爬坡, 2=尾部降速, 3=中间掉速（爬坡限距10s）
        _ramp_type = pd.Series(0, index=df.index)
        _ramp_type[(_ramp_full & (df['_t'] < first_t) & (df['_t'] >= first_t - timedelta(seconds=10)))] = 1
        _ramp_type[(_ramp_full & (df['_t'] > last_t) & (df['_t'] <= last_t + timedelta(seconds=10)))] = 2
        _ramp_type[(_ramp_full & (df['_t'] >= first_t) & (df['_t'] <= last_t) & ~_active_core)] = 3
        # 存储爬坡数据供汇总使用
        seg['_ramp_first_t'] = df.loc[_ramp_full, '_t'].min() if _ramp_full.any() else first_t
        seg['_ramp_last_t'] = df.loc[_ramp_full, '_t'].max() if _ramp_full.any() else last_t
        seg['_ramp_mask'] = _ramp_full
        seg['_ramp_type'] = _ramp_type

    # FTP配对时长对齐：长短边差值>0.6s时，裁剪长边首尾低值
    for f1, f2 in fp:
        dur1 = f1.get('mark_dur', 0); dur2 = f2.get('mark_dur', 0)
        if dur1 <= 0 or dur2 <= 0: continue
        diff = abs(dur1 - dur2)
        if diff <= 0.6: continue
        # 确定长边和短边
        _long, _short = (f1, f2) if dur1 > dur2 else (f2, f1)
        _long_dur, _short_dur = max(dur1, dur2), min(dur1, dur2)
        # 长边裁剪：去掉首/尾diff秒中速率更低的那些行
        _lcol = '_dl' if _long['业务'] in DL_BIZ else '_ul'
        _lmask = (df['_t'] >= _long['mark_st']) & (df['_t'] <= _long['mark_et']) & (df['QCI'].isin(qci_list))
        _lvals = df.loc[_lmask, _lcol].fillna(0)
        if len(_lvals) <= _short_dur: continue
        _trim_n = int(len(_lvals) - _short_dur)
        if _trim_n <= 0: continue
        # 从首尾各取_trim_n个最低值的位置移除
        # 从首尾各取_trim_n个候选值，合并后取最小的_trim_n个
        _candidates = pd.concat([_lvals.head(_trim_n), _lvals.tail(_trim_n)])
        _trim_idx = set(_candidates.nsmallest(_trim_n).index)
        if _trim_idx:
            _new_mask = _lmask & ~df.index.isin(_trim_idx)
            if _new_mask.any():
                # 更新I列和速率
                df.loc[_lmask, '业务识别'] = None  # 先清掉旧的
                df.loc[_new_mask, '业务识别'] = _long['业务']
                _new_t = df.loc[_new_mask, '_t']
                _new_dur = (_new_t.max() - _new_t.min()).total_seconds()
                df.loc[_new_mask, '持续时长'] = round(_new_dur, 3)
                if _new_dur > 0:
                    _long_col = '下载速率' if _long['业务'] in DL_BIZ else '上传速率'
                    _vs = pd.to_numeric(df.loc[_new_mask, _long_col], errors='coerce').fillna(0).tolist()
                    df.loc[_new_mask, '速率(Mbps)'] = round(sum(_vs) / _new_dur, 2)
                _long['mark_st'], _long['mark_et'], _long['mark_dur'] = _new_t.min(), _new_t.max(), _new_dur
    ...
    # 二次清理：首FTP对之前的所有行，业务标记全部清除
    if pi:
        _first_pair_start = pi[0][2]  # 第一对FTP候选的开始时间
        _pre_mask = (df['_t'] < _first_pair_start) & (df['业务识别'].notna())
        if _pre_mask.any():
            df.loc[_pre_mask, '业务识别'] = '未知'
            df.loc[_pre_mask, '速率(Mbps)'] = None
            df.loc[_pre_mask, '持续时长'] = None
            df.loc[_pre_mask, '轮次'] = None

    # ===== 段边界回填 + 中间掉速补标 =====
    # 0) 同一秒内的CA配对行：主小区行已标记，辅小区行直接继承
    _same_sec = df.groupby(df['_t'].dt.floor('s'))
    for _sec, _grp in _same_sec:
        _marked = _grp[_grp['业务识别'].notna()]
        _unmarked = _grp[_grp['业务识别'].isna()]
        if len(_marked) > 0 and len(_unmarked) > 0:
            _biz = _marked['业务识别'].iloc[0]
            _rnd = _marked['轮次'].iloc[0] if '轮次' in _marked.columns else None
            if _biz not in (None, '未知', ''):
                df.loc[_unmarked.index, '业务识别'] = _biz
                if _rnd is not None:
                    df.loc[_unmarked.index, '轮次'] = _rnd
    # 1) 段首尾±3s内速率>10的未标记行 → 补标为同一业务
    # 2) 段中间被漏掉的行 → 补标
    # 3) 速率<峰值30%的行 → 标为对应业务
    # 4) "未知"行若紧邻已知业务且速率>50 → 改标为相邻业务
    _biz_rows = df[df['业务识别'].notna()].index
    if len(_biz_rows) > 0:
        # 4) 先处理"未知"行：相邻±2s内有已知业务且自身速率>50的，改标为已知业务
        _unknown_mask = (df['业务识别'] == '未知') & qci_mask
        if _unknown_mask.any():
            _known_mask = df['业务识别'].notna() & (df['业务识别'] != '未知')
            for _ui in df[_unknown_mask].index:
                _ut = df.loc[_ui, '_t']
                _nearby = df[_known_mask & (df['_t'] >= _ut - timedelta(seconds=5)) & (df['_t'] <= _ut + timedelta(seconds=5))]
                if len(_nearby) > 0:
                    _nb = _nearby['业务识别'].iloc[0]
                    _nr = _nearby['轮次'].iloc[0] if '轮次' in _nearby.columns else None
                    _ncol = '_dl' if _nb in DL_BIZ else '_ul'
                    _nval = df.loc[_ui, _ncol] if _ui in df.index else 0
                    if pd.notna(_nval) and _nval > 50:
                        df.loc[_ui, '业务识别'] = _nb
                        if _nr is not None:
                            df.loc[_ui, '轮次'] = _nr
        # 按业务+轮次分组，找每组的边界
        _groups = df.loc[_biz_rows].groupby(['业务识别', '轮次'], dropna=False)
        for (_biz, _rnd), _grp in _groups:
            if _biz in (None, '未知', ''): continue
            _grp_t = _grp['_t'].sort_values()
            _seg_st, _seg_et = _grp_t.min(), _grp_t.max()
            _biz_col = '_dl' if _biz in DL_BIZ else '_ul'
            _peak = df.loc[_grp.index, _biz_col].max()
            _dip_th = _peak * 0.3 if _peak > 0 else 0
            # 扩展范围：段首尾各±3s
            _ext_st = _seg_st - timedelta(seconds=3)
            _ext_et = _seg_et + timedelta(seconds=3)
            _ext_mask = (df['_t'] >= _ext_st) & (df['_t'] <= _ext_et) & qci_mask & (df['业务识别'].isna())
            if _ext_mask.any():
                _ext_vals = pd.to_numeric(df.loc[_ext_mask, _biz_col], errors='coerce').fillna(0)
                _ext_valid = _ext_mask & (_ext_vals > 10.0)
                if _ext_valid.any():
                    df.loc[_ext_valid, '业务识别'] = _biz
                    df.loc[_ext_valid, '轮次'] = _rnd
            # 中间漏掉的行（段内未标记但速率>10）：补标
            _inner_mask = (df['_t'] >= _seg_st) & (df['_t'] <= _seg_et) & qci_mask & (df['业务识别'].isna())
            if _inner_mask.any():
                _inner_vals = pd.to_numeric(df.loc[_inner_mask, _biz_col], errors='coerce').fillna(0)
                _inner_valid = _inner_mask & (_inner_vals > 10.0)
                if _inner_valid.any():
                    df.loc[_inner_valid, '业务识别'] = _biz
                    df.loc[_inner_valid, '轮次'] = _rnd
            # 速率<峰值30%的行：已有业务标记，确保不掉（它们已在段内）
            if _dip_th > 0:
                _dip_mask = (df['_t'] >= _seg_st) & (df['_t'] <= _seg_et) & qci_mask & (df['业务识别'].isna())
                if _dip_mask.any():
                    _dip_vals = pd.to_numeric(df.loc[_dip_mask, _biz_col], errors='coerce').fillna(0)
                    _dip_add = _dip_mask & (_dip_vals > 0) & (_dip_vals <= _dip_th)
                    if _dip_add.any():
                        df.loc[_dip_add, '业务识别'] = _biz
                        df.loc[_dip_add, '轮次'] = _rnd

    # 爬坡标记：将各段的爬坡掩码合并到DataFrame
    if True:  # 爬坡/掉速标记始终执行，对勾只控制汇总是否纳入计算
        df['_ramp_type'] = 0
        for s in valid:
            if '_ramp_type' in s and s['_ramp_type'] is not None:
                _rt = s['_ramp_type']
                df.loc[_rt[_rt > 0].index, '_ramp_type'] = _rt[_rt > 0]
        # 扩展业务标记到爬坡行
        for s in valid:
            if '_ramp_mask' in s and s['_ramp_mask'] is not None:
                _rm = s['_ramp_mask'] & (df['_ramp_type'] > 0) & (df['业务识别'].isna())
                if _rm.any():
                    df.loc[_rm, '业务识别'] = s['业务']
                    df.loc[_rm, '持续时长'] = round((s.get('_ramp_last_t', s['mark_et']) - s.get('_ramp_first_t', s['mark_st'])).total_seconds(), 3)
                    _rcol = '_dl' if s['业务'] in DL_BIZ else '_ul'
                    _rvs = pd.to_numeric(df.loc[_rm, '下载速率'] if s['业务'] in DL_BIZ else df.loc[_rm, '上传速率'], errors='coerce').fillna(0).tolist()
                    _rdur = (s.get('_ramp_last_t', s['mark_et']) - s.get('_ramp_first_t', s['mark_st'])).total_seconds()
                    df.loc[_rm, '速率(Mbps)'] = round(sum(_rvs) / _rdur, 2) if _rdur > 0 else 0

    # ===== UL流量回判（DL回判之前，保证原始标记未被清理）=====
    # DL<3的纯UL连续块，累积UL总量>1700→微信大包发送
    if pi:
        _ul_only = (df['_dl'].fillna(0) < 3) & (df['_ul'].fillna(0) > 3) & qci_mask & (df['_t'] >= pi[0][2])
    else:
        _ul_only = (df['_dl'].fillna(0) < 3) & (df['_ul'].fillna(0) > 3) & qci_mask
    _known_biz = df['业务识别'].notna() & (df['业务识别'] != '未知') & ~df['业务识别'].astype(str).str.contains('-爬坡|-掉速', na=False)
    if _ul_only.any():
        _uo_idx = df[_ul_only].index.tolist()
        _blocks = []; _cur = [_uo_idx[0]]
        for _i in range(1, len(_uo_idx)):
            _gap = (df.loc[_uo_idx[_i], '_t'] - df.loc[_uo_idx[_i-1], '_t']).total_seconds()
            if _gap <= 10: _cur.append(_uo_idx[_i])
            else: _blocks.append(_cur); _cur = [_uo_idx[_i]]
        _blocks.append(_cur)
        for _blk in _blocks:
            _blk_ul_sum = df.loc[_blk, '_ul'].sum()
            _blk_start_t = df.loc[_blk[0], '_t']
            _prev_biz_rows = df[_known_biz & (df['_t'] < _blk_start_t)]
            _prev_biz = _prev_biz_rows['业务识别'].iloc[-1] if len(_prev_biz_rows) > 0 else ''
            _prev_ok = _prev_biz in ('微信小包发送', 'FTP上传')
            if WX_L_FLOW_MIN <= _blk_ul_sum <= WX_L_FLOW_MAX and _prev_ok:
                df.loc[_blk, '业务识别'] = '微信大包发送'
                _blk_dur = (df.loc[_blk[-1], '_t'] - df.loc[_blk[0], '_t']).total_seconds()
                df.loc[_blk, '持续时长'] = round(_blk_dur, 3)
                if _blk_dur > 0:
                    df.loc[_blk, '速率(Mbps)'] = round(_blk_ul_sum / _blk_dur, 2)
                _wx_start = df.loc[_blk[0], '_t']
                _prev_biz_t = df[_known_biz & (df['_t'] < _wx_start)]['_t'].max()
                _scan_start = _prev_biz_t if pd.notna(_prev_biz_t) else _wx_start - timedelta(seconds=60)
                _pre_mask = _ul_only & (df['_t'] >= _scan_start) & (df['_t'] < _wx_start) & ~df.index.isin(_blk)
                if _pre_mask.any():
                    _pre_idx = df[_pre_mask].index.tolist()
                    _pre_blocks = []; _cur = [_pre_idx[0]]
                    for _i in range(1, len(_pre_idx)):
                        _pg = (df.loc[_pre_idx[_i], '_t'] - df.loc[_pre_idx[_i-1], '_t']).total_seconds()
                        if _pg <= 3: _cur.append(_pre_idx[_i])
                        else: _pre_blocks.append(_cur); _cur = [_pre_idx[_i]]
                    _pre_blocks.append(_cur)
                    for _pb in _pre_blocks:
                        _pb_ul = df.loc[_pb, '_ul'].sum()
                        _pb_dur = (df.loc[_pb[-1], '_t'] - df.loc[_pb[0], '_t']).total_seconds()
                        if WX_S_FLOW_MIN <= _pb_ul <= WX_S_FLOW_MAX and 0.5 <= _pb_dur <= 5:
                            df.loc[_pb, '业务识别'] = '微信小包发送'
                            df.loc[_pb, '持续时长'] = round(_pb_dur, 3)
                            if _pb_dur > 0:
                                df.loc[_pb, '速率(Mbps)'] = round(_pb_ul / _pb_dur, 2)
                            break

    # ===== DL流量回判（近零/方向清理之前，保留原始标记）=====
    # 注意：不含'应用商店'核心行——已被窗口扫描/状态机标记的大/小文件核心行不再回判，避免把大文件段腰斩成小文件
    _dl_only = (df['_ul'].fillna(0) < 5) & (df['_dl'].fillna(0) > 3) & qci_mask & (
        df['业务识别'].isna() | (df['业务识别'] == '未知') |
        df['业务识别'].astype(str).str.contains('-爬坡|-掉速', na=False))
    if _dl_only.any() and '微信大包发送' in df['业务识别'].values:
        _wx_all = df[df['业务识别'] == '微信大包发送']['_t'].sort_values().tolist()
        _wx_blocks = []; _cur = [_wx_all[0]]
        for _i in range(1, len(_wx_all)):
            if (_wx_all[_i] - _wx_all[_i-1]).total_seconds() <= 60: _cur.append(_wx_all[_i])
            else: _wx_blocks.append(_cur[0]); _cur = [_wx_all[_i]]
        _wx_blocks.append(_cur[0])
        for _wxt in _wx_blocks:
            _store_big_rows = df[(df['业务识别'].notna()) &
                                 (df['业务识别'].astype(str).str.startswith('应用商店大文件下载')) &
                                 (df['_t'] > _wxt)].sort_values('_t')
            if len(_store_big_rows) > 0:
                _sbt = _store_big_rows['_t']
                _next_bound = _sbt.iloc[0]
                for _i in range(1, len(_sbt)):
                    if (_sbt.iloc[_i] - _sbt.iloc[_i-1]).total_seconds() <= 60:
                        _next_bound = _sbt.iloc[_i]
                    else: break
                _next_bound += timedelta(seconds=1)
            else:
                _next_bound = df[(df['业务识别'].notna()) &
                                 (df['业务识别'].astype(str).str.startswith('FTP下载')) &
                                 (df['_t'] > _wxt)]['_t'].min()
            if pd.isna(_next_bound): continue
            _dl_candidates = _dl_only & (df['_t'] > _wxt) & (df['_t'] <= _next_bound)
            if not _dl_candidates.any(): continue
            _dl_idx = df[_dl_candidates].index.tolist()
            _dl_blocks = []; _cur = [_dl_idx[0]]
            for _i in range(1, len(_dl_idx)):
                _gap = (df.loc[_dl_idx[_i], '_t'] - df.loc[_dl_idx[_i-1], '_t']).total_seconds()
                _dl_cur = df.loc[_dl_idx[_i], '_dl']
                _dl_prev = df.loc[_dl_idx[_i-1], '_dl']
                _dl_jump = _dl_prev > 50 and _dl_cur > _dl_prev * 2 and _dl_cur > 500
                if _gap <= 5 and not _dl_jump: _cur.append(_dl_idx[_i])
                else: _dl_blocks.append(_cur); _cur = [_dl_idx[_i]]
            _dl_blocks.append(_cur)
            for _bi, _blk in enumerate(_dl_blocks):
                _blk_dl_sum = df.loc[_blk, '_dl'].sum()
                _merged_prev = False
                if _bi > 0:
                    _prev_blk = _dl_blocks[_bi - 1]
                    _prev_dl = df.loc[_prev_blk, '_dl'].sum()
                    _merged_prev = False
                    if _prev_dl < STORE_S_FLOW_MIN:
                        _blk_dl_sum += df.loc[_prev_blk, '_dl'].sum()
                        _merged_prev = True
                if STORE_S_FLOW_MIN < _blk_dl_sum < STORE_L_FLOW_MIN:
                    if _merged_prev:
                        df.loc[_prev_blk, '业务识别'] = '应用商店小文件下载'
                    df.loc[_blk, '业务识别'] = '应用商店小文件下载'
                elif _blk_dl_sum >= STORE_L_FLOW_MIN:
                    df.loc[_blk, '业务识别'] = '应用商店大文件下载'
                _blk_dur = (df.loc[_blk[-1], '_t'] - df.loc[_blk[0], '_t']).total_seconds()
                df.loc[_blk, '持续时长'] = round(_blk_dur, 3)
                if _blk_dur > 0:
                    df.loc[_blk, '速率(Mbps)'] = round(_blk_dl_sum / _blk_dur, 2)

    # 商店大文件结束→下一个FTP之间：用每块大文件的末行做边界，清除全部业务标记
    _store_big_end = df[df['业务识别'].astype(str).str.startswith('应用商店大文件下载') & (df['业务识别'].notna())]
    if len(_store_big_end) > 0:
        _sbe_times = _store_big_end['_t'].sort_values()
        _sbe_blocks = []; _cur_s = [_sbe_times.iloc[0]]
        for _i in range(1, len(_sbe_times)):
            if (_sbe_times.iloc[_i] - _sbe_times.iloc[_i-1]).total_seconds() <= 60:
                _cur_s.append(_sbe_times.iloc[_i])
            else: _sbe_blocks.append(_cur_s[-1]); _cur_s = [_sbe_times.iloc[_i]]
        _sbe_blocks.append(_cur_s[-1])
        for _sbe in _sbe_blocks:
            _next_ftp = df[(df['业务识别'].notna()) &
                           (df['业务识别'].astype(str).str.startswith('FTP上传')) &
                           (df['_t'] > _sbe)]['_t'].min()
            if pd.notna(_next_ftp):
                _clear_mask = (df['_t'] > _sbe) & (df['_t'] < _next_ftp) & (df['业务识别'].notna())
                if _clear_mask.any():
                    df.loc[_clear_mask, '业务识别'] = '未知'
                    df.loc[_clear_mask, '速率(Mbps)'] = None
                    df.loc[_clear_mask, '持续时长'] = None

    # 近零清理：DL<5且UL<5的行不标记
    _near_zero = (df['_dl'].fillna(0) < 5) & (df['_ul'].fillna(0) < 5) & (df['业务识别'].notna())
    if _near_zero.any():
        df.loc[_near_zero, '业务识别'] = None
        df.loc[_near_zero, '速率(Mbps)'] = None
        df.loc[_near_zero, '持续时长'] = None
        df.loc[_near_zero, '轮次'] = None

    # 方向不匹配清理：DL业务行DL<3 或 UL业务行UL<3 → 改标"未知"
    for _biz_base, _dl_ul_flag in [('FTP下载', 'dl'), ('应用商店大文件下载', 'dl'), ('应用商店小文件下载', 'dl'),
                                     ('FTP上传', 'ul'), ('微信大包发送', 'ul'), ('微信小包发送', 'ul')]:
        _mismatch = (df['业务识别'].notna() & df['业务识别'].astype(str).str.startswith(_biz_base) &
                     (df['_dl' if _dl_ul_flag == 'dl' else '_ul'].fillna(0) < 3))
        if _mismatch.any():
            df.loc[_mismatch, '业务识别'] = '未知'
            df.loc[_mismatch, '速率(Mbps)'] = None
            df.loc[_mismatch, '持续时长'] = None
    ...
    def _process_one_operator(df_op, op_name, base_op, has_base_op):
        ...
        #   含爬坡：无后缀 + "-爬坡" (排除 "-掉速")
        #   含爬坡+掉速：全部
        _all_biz = df_detail['businessType'].notna()
        _no_suffix = _all_biz & ~df_detail['businessType'].astype(str).str.contains('-爬坡|-掉速', na=False)
        _with_ramp = _all_biz & ~df_detail['businessType'].astype(str).str.contains('-掉速', na=False)
        _with_all = _all_biz

        _detail_no_ramp = df_detail[_no_suffix]
        df_summary_all = _calc_summary(_detail_no_ramp, dl_clip, ul_clip)
        complete_mask = _detail_no_ramp['轮次'].astype(str).str.isdigit()
        df_detail_complete = _detail_no_ramp[complete_mask]
        df_summary_complete = _calc_summary(df_detail_complete, dl_clip, ul_clip)

        _sum_ramp_all = None; _sum_ramp_comp = None
        if calc_ramp:
            _d_ramp = df_detail[_with_ramp]
            _d_ramp_comp = _d_ramp[_d_ramp['轮次'].astype(str).str.isdigit()]
            _sum_ramp_all = _calc_summary(_d_ramp, dl_clip, ul_clip)
            _sum_ramp_comp = _calc_summary(_d_ramp_comp, dl_clip, ul_clip)

        _sum_ramp_mid_all = None; _sum_ramp_mid_comp = None
        if calc_ramp_mid:
            _d_mid = df_detail[_with_all]
            _d_mid_comp = _d_mid[_d_mid['轮次'].astype(str).str.isdigit()]
            _sum_ramp_mid_all = _calc_summary(_d_mid, dl_clip, ul_clip)
            _sum_ramp_mid_comp = _calc_summary(_d_mid_comp, dl_clip, ul_clip)
        ...
        return {
            ...
            '_sum_ramp_all': _sum_ramp_all,
            '_sum_ramp_comp': _sum_ramp_comp,
            '_sum_ramp_mid_all': _sum_ramp_mid_all,
            '_sum_ramp_mid_comp': _sum_ramp_mid_comp,
        }
    ...
    with xlsxwriter.Workbook(_out_tmp, {'strings_to_numbers': False, 'nan_inf_to_errors': True, 'default_date_format': 'yyyy-mm-dd hh:mm:ss'}) as wb:
        ...
        fmt_thick_bottom = wb.add_format({'bottom': 2})  # 表对间粗分隔线
        ...
        for op in operators_list:
            ...
            _sum_ramp_all = res.get('_sum_ramp_all')
            _sum_ramp_comp = res.get('_sum_ramp_comp')
            _sum_ramp_mid_all = res.get('_sum_ramp_mid_all')
            _sum_ramp_mid_comp = res.get('_sum_ramp_mid_comp')
            ...
            for i in range(n_detail):
                ...
                # D/E列按业务方向着色（蓝=DL, 绿=UL），爬坡后缀不影响颜色
                _biz_base = _biz_str.replace('-爬坡','').replace('-掉速','') if _has_biz else ''
                _is_dl_biz = _biz_base in DL_BIZ if _has_biz else False
                _is_ul_biz = _biz_base in UL_BIZ if _has_biz else False
                _biz_color = fmt_biz.get(_biz_base, fmt_d_s) if _has_biz else None
                if n_col_detail > col_3_idx:
                    if d_dash:
                        ws.write_string(row0, col_3_idx, '-')
                    else:
                        _dv = float(_d_arr[i]) if _d_arr is not None else 0
                        if not pd.isna(_dv): ws.write_number(row0, col_3_idx, _dv, _biz_color_num.get(_biz_base, _fmt_d_no_color) if _is_dl_biz else _fmt_d_no_color)
                if n_col_detail > col_4_idx:
                    if e_dash:
                        ws.write_string(row0, col_4_idx, '-')
                    else:
                        _ev = float(_e_arr[i]) if _e_arr is not None else 0
                        if not pd.isna(_ev): ws.write_number(row0, col_4_idx, _ev, _biz_color_num.get(_biz_base, _fmt_e_no_color) if _is_ul_biz else _fmt_e_no_color)
            ...
            # 原表对末行加粗底框
            for _ci in range(9):
                ws_sum.write(last_row_sum + 1, _ci, '', fmt_thick_bottom)
            _next_row = last_row_sum + 3  # 空2行+粗线行

            # 爬坡表：堆叠写入同一个Sheet
            def _append_ramp_tables(label, sum_all, sum_comp, start_row):
                r0 = start_row
                ws_sum.merge_range(r0, 0, r0, 8, f'▼ 所有识别业务（{label}）', fmt_ta)
                for ri in range(len(sum_all)):
                    for ci in range(9):
                        cv = sum_all.iloc[ri, ci] if ci < len(sum_all.columns) else None
                        if isinstance(cv, float) and pd.isna(cv): cv = None
                        _ws_write(ws_sum, r0 + 1 + ri, ci, cv, fmt_d_s)
                r1 = r0 + 1 + len(sum_all) + 2
                ws_sum.merge_range(r1, 0, r1, 8, f'▼ 仅完整轮次（代码侧6业务齐全，{label}）', fmt_tc)
                for ri in range(len(sum_comp)):
                    for ci in range(9):
                        cv = sum_comp.iloc[ri, ci] if ci < len(sum_comp.columns) else None
                        if isinstance(cv, float) and pd.isna(cv): cv = None
                        _ws_write(ws_sum, r1 + 1 + ri, ci, cv, fmt_d_s)
                _last = r1 + 1 + len(sum_comp)  # 表对末行
                for _ci in range(9):
                    ws_sum.write(_last, _ci, '', fmt_thick_bottom)
                return _last + 2  # 粗线行+1空行

            if calc_ramp and _sum_ramp_all is not None:
                _next_row = _append_ramp_tables('含爬坡', _sum_ramp_all, _sum_ramp_comp, _next_row)
            if calc_ramp_mid and _sum_ramp_mid_all is not None:
                _next_row = _append_ramp_tables('含爬坡+掉速', _sum_ramp_mid_all, _sum_ramp_mid_comp, _next_row)

            # 统一设行高 + 冻结
            for r in range(0, _next_row + 1):
                ws_sum.set_row(r, 22)
            ...
            def _add_comments(sub_detail, row_offset):
                ...
                for bi, biz in enumerate(sorted(DL_BIZ | UL_BIZ)):
                    ...
                    rates = pd.to_numeric(sub[cn], errors='coerce').fillna(0) / 1e6
                    # 子段修剪（与_calc_summary保持一致）
                    if len(rates) > 1:
                        _peak = rates.max()
                        if _peak > 0:
                            _th = max(_peak * 0.1, 5.0)
                            _mask = rates >= _th
                            _active_idx = rates.index[_mask]
                            if len(_active_idx) > 0:
                                rates = rates.loc[_active_idx[0]:_active_idx[-1]]
                    rates = rates[rates > 0]
                    ts2 = sub['_t']; durs = []
                    if len(ts2) > 0:
                        s1 = ts2.iloc[0]; p = s1
                    ...
                    for cell_col in range(2, 8):  # 1-based col 2..7
                        ...
                        if cp in (0, 1):
                            ...
                            if cp == 2:
                                ...
                                op_cn = f"SUM(rate) / COUNT(all)\n= 速率求和 / 总采样数"
                                calc = f"= {sum_v:.1f} / {n}"
                            elif cp == 3:
                                clipped = rates.clip(upper=lim)
                                mol = list(clipped); sum_v = sum(clipped)
                                sum_label = f"削峰后求和={sum_v:.1f}"; cnt_label = f"计数={n}"
                                op_cn = f"SUM(CLIP(rate, 0, {lim})) / COUNT(all)\n= 削峰后速率求和 / 总采样数"
                                calc = f"= {sum_v:.1f} / {n}"
                            else:
                                clipped_sv = rates.clip(upper=lim).sort_values(ascending=False)
                                tn = max(1, int(len(clipped_sv) * 0.1))
                                mol = list(clipped_sv.head(tn)); sum_v = sum(mol)
                                sum_label = f"TOP10%求和(已削峰)={sum_v:.1f}"; cnt_label = f"TOP10%计数={len(mol)}"
                                op_cn = f"SUM(CLIP(rate,0,{lim}) TOP10%) / COUNT(TOP10%)\n= 削峰后前10%速率求和 / 前10%个数"
                                calc = f"= {sum_v:.1f} / {len(mol)}"
                        else:
                            continue

                        label_str = f"{sum_label}; {cnt_label}" if 'sum_label' in locals() else cnt_label
                        txt = (f"公式: {op_cn}\n"
                               f"计算过程: {calc}\n"
                               f"结果: {cv}\n\n"
                               f"{label_str}\n"
                               f"值: {_fmt_vals(mol)}\n\n"
                               f"分子来源: {cn} 列, 第 {r_start}~{r_end} 行 (详见详细过程 {_gcl(cell_col)}{r_start})\n"
                               f"原始数据行数: {len(sub)}")
                        ws_sum.write_comment(row0, col0, txt, {'width': 500, 'height': 220})

                # 时长列批注 (col 7, 8 → 0-based 6, 7)
                for bi, biz in enumerate(sorted(DL_BIZ | UL_BIZ)):
                    sub = sub_detail[sub_detail['businessType'] == biz].dropna(subset=['_t']).sort_values('_t')
                    if len(sub) == 0: continue
                    ts2 = sub['_t']; dur_vals = []
                    ...
                    for cell_col in [7, 8]:
                        ...
                        if cell_col == 7:
                            txt = (f"AVERAGE(duration)\n= 业务段时间平均值\n"
                                   ...
                                   f"来源: 业务时长(秒) 列, 第 {r_start}~{r_end} 行 (详见详细过程 col_H{r_start})")
                        else:
        ...
        for biz in BIZ_ORDER:
            ...
            if _lt_data[0] is not None:
                ws_rlc.write_number(ri, 1, _lt_data[0], _fmt_rlc)
            if _lt_data[1] is not None:
                ws_rlc.write_number(ri, 2, _lt_data[1], _fmt_rlc)
            # 电信
            _dx_data = _rlc_data.get('电信', {}).get(biz, (None, None))
            ws_rlc.write(ri, 3, short_name, fmt_d_s)
            if _dx_data[0] is not None:
                ws_rlc.write_number(ri, 4, _dx_data[0], _fmt_rlc)
            if _dx_data[1] is not None:
                ws_rlc.write_number(ri, 5, _dx_data[1], _fmt_rlc)
            ri += 1
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None, biz_params=None, calc_ramp=False, calc_ramp_mid=False):
            super().__init__()
            ...
            self.calc_ramp = calc_ramp; self.calc_ramp_mid = calc_ramp_mid
        ...
        def _do_data_run(self):
            ...
            self.thread = WorkerThread(self.files, qci_list, self.sd.value(), self.su.value(),
                                       ...
                                       calc_ramp=self.cb_ramp.isChecked(),
                                       calc_ramp_mid=self.cb_ramp_mid.isChecked())
        ...
        def run(self):
            try:
                out = process(self.files, self.qci_list, self.dl_clip, self.ul_clip,
                              ...
                              calc_ramp=self.calc_ramp, calc_ramp_mid=self.calc_ramp_mid)
    ...
    class MainWindow(QMainWindow):
        ...
        def _build_rate_tab(self):
            ...
            # 结果文件包括：爬坡速率对勾（两个独立）
            _params_ramp = QHBoxLayout(); _params_ramp.setSpacing(6)
            _params_ramp.addWidget(QLabel("结果文件包括："))
            self.cb_ramp = QCheckBox("计算爬坡速率"); self.cb_ramp.setChecked(False)
            self.cb_ramp_mid = QCheckBox("计算爬坡速率（包括中间掉速值）"); self.cb_ramp_mid.setChecked(False)
            _params_ramp.addWidget(self.cb_ramp); _params_ramp.addWidget(self.cb_ramp_mid); _params_ramp.addStretch()
            _outer.addLayout(_params_ramp)
            # === 快速方案 + 业务顺序 ===
            _BIZ_SCHEMES = {
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>2026-07-22</b></p>"
                "<ul>"
                "<li><b>V2.5.38</b> 修复应用商店识别两个bug：①序列状态机_big_bizs补漏'应用商店大文件下载'；②商店窗口扫描重写——活跃门槛降到4M，逐秒累加≥900切小文件，后续跑到活跃秒末尾累计≥9000整段为大文件（不切在9000点）</li>\n"
                "<li><b>V2.5.37</b> 新增「用户级公共监控」格式输入支持（CSV/XLSX自动识别元数据头、中文列映射、CA辅小区过滤、速率Mbps→bps口径对齐）；时间解析兼容括号前带空格的格式；文件过滤不再限制MMF文件名和CSV扩展名；老MMF格式读取规则完整保留</li>\n"
