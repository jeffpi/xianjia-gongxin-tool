...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None):
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        ...
        # ===== 对比竖表（对比横表的转置：行列互换，便于竖向查看）=====
        ws_v = wb.create_sheet('对比竖表')
        for r in range(1, ws_h.max_row + 1):
            for c in range(1, ws_h.max_column + 1):
                v = ws_h.cell(row=r, column=c).value
                if v is not None:
                    ws_v.cell(row=c, column=r, value=v).alignment = center
        # 转置合并区域（行列互换）
        for mc in list(ws_h.merged_cells.ranges):
            ws_v.merge_cells(start_row=mc.min_col, start_column=mc.min_row,
                             end_row=mc.max_col, end_column=mc.max_row)
        # 自适应列宽
        from openpyxl.utils import get_column_letter as _gcl
        for cidx in range(1, ws_v.max_column + 1):
            mx = 0
            for ridx in range(1, min(ws_v.max_row + 1, 80)):
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("5G用户级公共监控速率统计工具 V2.2.0")
        ...
        def _build(self):
            ...
            title = QLabel("5G用户级公共监控速率统计工具  V2.2.0")
            ...
            ghl.addWidget(self.br); ghl.addWidget(self.babout); ll.addWidget(gh)
            ll.addStretch(1)
