            ...
            for _sec_idx, (_sec_title, _sec_fmt, _sub_df) in enumerate([
                ...
                for _biz in _bizs:
                    ...
                    ws_sum.write_formula(_rw, 1,
                        f'=IFERROR(COUNTIFS({_ds}!I:I,"{_biz}",{_ds}!{_rate_col}:{_rate_col},">{_lim}")/COUNTIFS({_ds}!I:I,"{_biz}",{_ds}!{_rate_col}:{_rate_col},">0")*100,0)', fmt_d_s)
                    # 5M/100M以下占比(%)
                    ws_sum.write_formula(_rw, 2,
                        f'=IFERROR(COUNTIFS({_ds}!I:I,"{_biz}",{_ds}!{_rate_col}:{_rate_col},"<{_lo}")/COUNTIFS({_ds}!I:I,"{_biz}",{_ds}!{_rate_col}:{_rate_col},">0")*100,0)', fmt_d_s)
                    # 平均速率(Mbps)
                    ws_sum.write_formula(_rw, 3,
                        f'=IFERROR(AVERAGEIFS({_ds}!{_rate_col}:{_rate_col},{_ds}!I:I,"{_biz}"),0)', fmt_d_s)
                    # 削峰平均速率(Mbps) — CLIP到阈值
                    ws_sum.write_array_formula(_rw, 4, _rw, 4,
                        f'{{=IFERROR(AVERAGE(IF(({_ds}!I2:I{_max_r}="{_biz}")*({_ds}!{_rate_col}2:{_rate_col}{_max_r}>0),IF({_ds}!{_rate_col}2:{_rate_col}{_max_r}>{_lim},{_lim},{_ds}!{_rate_col}2:{_rate_col}{_max_r}))),0)}}')
                    # 削峰TOP10%峰值速率
                    ws_sum.write_array_formula(_rw, 5, _rw, 5,
                        f'{{=IFERROR(AVERAGE(LARGE(IF(({_ds}!I2:I{_max_r}="{_biz}")*({_ds}!{_rate_col}2:{_rate_col}{_max_r}>0),IF({_ds}!{_rate_col}2:{_rate_col}{_max_r}>{_lim},{_lim},{_ds}!{_rate_col}2:{_rate_col}{_max_r})),ROW(INDIRECT("1:"&MAX(1,ROUND(COUNTIFS({_ds}!I:I,"{_biz}",{_ds}!{_rate_col}:{_rate_col},">0")*0.1,0)))))),0)}}')
                    # 业务时长平均值(s)
                    ws_sum.write_formula(_rw, 6,
                        f'=IFERROR(AVERAGEIFS({_ds}!G:G,{_ds}!I:I,"{_biz}"),0)', fmt_d_s)
                    # 业务时长中位值(s) — 数组公式
                    ws_sum.write_array_formula(_rw, 7, _rw, 7,
                        f'{{=IFERROR(MEDIAN(IF({_ds}!I2:I{_max_r}="{_biz}",{_ds}!G2:G{_max_r})),0)}}')
                    # 手机号 — 保持直接值（无法用简单公式表达）
