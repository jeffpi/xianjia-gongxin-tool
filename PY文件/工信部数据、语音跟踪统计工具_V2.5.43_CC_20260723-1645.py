...
# 默认全集（方案一/二使用），方案三由 process() 内从 BIZ_ORDER 动态推导
_DL_BIZ_FULL = {'FTP下载', '应用商店小文件下载', '应用商店大文件下载'}
_UL_BIZ_FULL = {'FTP上传', '微信小包发送', '微信大包发送'}
...
def _find_time_in_paths(dirs, files):
    """从目录名和文件路径中提取时间范围。优先目录名，其次向上遍历祖先目录(5层)，最后文件名"""
    # 1. 检查拖入的目录名
    for d in dirs:
        ...
        if result:
            return result
    # 2. 检查文件路径：向上遍历祖先目录（5层）→ 文件名
    for f in files:
        p = os.path.dirname(f)
        for _ in range(5):  # 最多向上5层
            parent = os.path.basename(p)
...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None, biz_params=None):
    ...
    # 从 BIZ_ORDER 动态推导 DL_BIZ/UL_BIZ（支持方案三只有FTP的场景）
    DL_BIZ = {b for b in BIZ_ORDER if '下载' in b}
    UL_BIZ = {b for b in BIZ_ORDER if '上传' in b or '发送' in b}
    _is_ftp_only = len(BIZ_ORDER) == 2 and 'FTP上传' in BIZ_ORDER and 'FTP下载' in BIZ_ORDER
    ...
    for s in _big_sorted:
        ...
        _found = False
        _big_bizs = {'FTP下载', 'FTP上传'} if _is_ftp_only else {'FTP下载', 'FTP上传', '应用商店大文件下载', '微信大包发送'}
        ...
        # ⑤ 兜底：方案三(FTP only)直接标未知，其他先尝试应用商店匹配
        if _is_ftp_only:
            s['biz'] = '未知'
        elif _match_strict(s, '应用商店小文件下载') or _match_relaxed(s, '应用商店小文件下载'):
            s['biz'] = '应用商店小文件下载'
    ...
    for s in fc:
        if id(s) not in paired:
            if not _is_ftp_only and STORE_L_DL_MIN <= s['dl'] <= STORE_L_DL_MAX and s['dl'] > s['ul']: s['biz'] = '应用商店大文件下载'
            else: s['biz'] = fd(s)
    ...
    if not fp:
        ...
        _large_biz_names = ('FTP下载', 'FTP上传') if _is_ftp_only else ('应用商店大文件下载', '微信大包发送', 'FTP下载', 'FTP上传')
        large_bizs = [s for s in al if s['业务'] in _large_biz_names]
    ...
    _tb_bizs = ('FTP下载', 'FTP上传') if _is_ftp_only else ('FTP下载', 'FTP上传', '应用商店大文件下载', '微信大包发送')
    tb = sorted([s for s in al if s['业务'] in _tb_bizs], key=lambda x: x['st'])
    # 小业务检测（方案三无此需要，跳过）
    small = []
    if not _is_ftp_only:
    ...
    # 检测第一个FTP对之前的不完整轮次（方案三跳过）
    if not _is_ftp_only and pi:
    ...
    rd = defaultdict(set)
    REQ_BIZ = set(BIZ_ORDER)  # 方案三只有2个业务时，完整轮次只需这2个
    for s in valid:
        r = s.get('rnd')
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("工信部数据、语音跟踪统计工具 V2.5.43_CC")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("工信部数据统计工具 V2.5.43_CC")
            ...
            _SCHEME_ROW_COUNT = {'方案一': 6, '方案二': 6, '方案三': 2}
            _ALL_BIZ = ['FTP上传', 'FTP下载', '微信小包发送', '微信大包发送', '应用商店小文件下载', '应用商店大文件下载']
            self._scheme_settings = QSettings("HubeiTool", "5GDataVoice")
            _scheme_settings = self._scheme_settings
            # 恢复上次选择的方案
            _last_scheme = _scheme_settings.value("scheme/current", "方案一")
            if _last_scheme not in _BIZ_SCHEMES: _last_scheme = "方案一"
            self._current_biz_scheme = _last_scheme
            # 恢复各方案的自定义顺序（如果有）
            for _sn in _BIZ_SCHEMES:
                _saved_order = _scheme_settings.value(f"scheme/{_sn}/order")
                if _saved_order:
                    _BIZ_SCHEMES[_sn] = list(_saved_order)
            # 快速方案一行
            _scheme_label_row = QHBoxLayout(); _scheme_label_row.setSpacing(4)
            _scheme_label_row.addWidget(QLabel("快速方案:"))
            _btn_s1 = QPushButton("方案一"); _btn_s2 = QPushButton("方案二"); _btn_s3 = QPushButton("方案三")
            _scheme_label_row.addWidget(_btn_s1); _scheme_label_row.addWidget(_btn_s2); _scheme_label_row.addWidget(_btn_s3); _scheme_label_row.addStretch()
            ...
            _biz_table_h = 6 * _row_h + _hdr_h + 6  # 初始6行高度（方案一/二）
            ...
            # 填充表格行（根据方案动态填充，恢复已保存的参数）
            def _fill_table(scheme_name):
                n = _SCHEME_ROW_COUNT.get(scheme_name, 6)
                self.biz_order_table.setRowCount(0)
                order = _BIZ_SCHEMES[scheme_name]
                for r in range(n):
                    self.biz_order_table.insertRow(r)
                    _biz_name = order[r]
                    _item = QTableWidgetItem(_biz_name)
                    self.biz_order_table.setItem(r, 0, _item)
                    for c in range(1, 7):
                        # 恢复已保存的参数
                        _saved_val = _scheme_settings.value(f"scheme/{scheme_name}/params/{_biz_name}/{c}", "")
                        _item = QTableWidgetItem(str(_saved_val) if _saved_val else "")
                        _item.setTextAlignment(Qt.AlignCenter)
                        self.biz_order_table.setItem(r, c, _item)
                # 调整表格高度
                _h = n * _row_h + _hdr_h + 6
                self.biz_order_table.setMinimumHeight(_h)
                self.biz_order_table.setMaximumHeight(_h + 4)
            # 监听表格修改，自动保存参数
            self.biz_order_table.itemChanged.connect(lambda item: self._save_biz_params_for_cell(item.row(), item.column()))
            _fill_table(self._current_biz_scheme)
            ...
            def _apply_scheme(name):
                """切换方案：刷新表格行数、顺序，保存当前方案到QSettings"""
                self._current_biz_scheme = name
                _scheme_settings.setValue("scheme/current", name)
                _fill_table(name)
                _highlight_scheme_btn(name)
            def _highlight_scheme_btn(name):
                ...
                _btn_s3.setStyleSheet("background-color: #c8e6c9; font-weight: bold;" if name == '方案三' else "")
            def _save_current_order():
                """保存当前方案的业务顺序到QSettings"""
                _name = self._current_biz_scheme
                _order = []
                for r in range(self.biz_order_table.rowCount()):
                    _item = self.biz_order_table.item(r, 0)
                    if _item and _item.text(): _order.append(_item.text())
                if _order: _scheme_settings.setValue(f"scheme/{_name}/order", _order)
            _btn_s1.clicked.connect(lambda: _apply_scheme('方案一'))
            _btn_s2.clicked.connect(lambda: _apply_scheme('方案二'))
            _btn_s3.clicked.connect(lambda: _apply_scheme('方案三'))
            _apply_scheme(_last_scheme)
            vs.addWidget(gp)
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("工信部语音VQI工具 V2.5.43_CC")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            vs = QSplitter(Qt.Vertical); vs.setChildrenCollapsible(False)
            if sys.platform == 'win32':
                vs.setStyleSheet("QSplitter::handle:vertical { background: #c8c8c8; height: 3px; margin: 1px 0; }")
        ...
        def _move_biz_item(self, direction):
            ...
            if direction == -1:  # 上移
                ...
                for r in rows:
                    self.biz_order_table.selectRow(r + 1)
            # 保存移动后的顺序到QSettings
            self._save_biz_order()

        def _save_biz_order(self):
            """保存当前方案的业务顺序到QSettings"""
            if not hasattr(self, '_scheme_settings'): return
            _name = self._current_biz_scheme
            _order = []
            for r in range(self.biz_order_table.rowCount()):
                _item = self.biz_order_table.item(r, 0)
                if _item and _item.text(): _order.append(_item.text())
            if _order:
                self._scheme_settings.setValue(f"scheme/{_name}/order", _order)

        def _save_biz_params_for_cell(self, row, col):
            """保存单个单元格的业务参数到QSettings"""
            if not hasattr(self, '_scheme_settings'): return
            _name = self._current_biz_scheme
            _item = self.biz_order_table.item(row, 0)
            _biz = _item.text() if _item else ''
            if not _biz: return
            _val_item = self.biz_order_table.item(row, col)
            _val = _val_item.text().strip() if _val_item else ''
            self._scheme_settings.setValue(f"scheme/{_name}/params/{_biz}/{col}", _val)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.43_CC &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p>"
                "<p><b>鸣谢：</b>丁先甲，谭润林，赵永川，廖洋（排名不分先后）</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-23</b></p>"
                "<ul>"
                "<li><b>V2.5.43_CC</b> 新增方案三（FTP only模式）：仅FTP上传/下载两业务，状态机跳过微信/应用商店逻辑；DL_BIZ/UL_BIZ从BIZ_ORDER动态推导；三方案全部持久化（QSettings保存方案选择+业务顺序+参数）</li>\n"
                "<li><b>V2.5.42_CC</b> 修复时间自动识别bug：_find_time_in_paths增加向上遍历祖先目录（5层），拖入深层子目录或选择文件时也能从上层目录名提取时间</li>\n"
