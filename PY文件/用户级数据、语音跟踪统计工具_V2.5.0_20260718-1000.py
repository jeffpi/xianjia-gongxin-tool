...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None):
    ...
    for f in voice_files:
        ...
        for df_i in voice_dfs:
            if f in df_i.get('_source_file', []):
                ...
                if len(times) > 0:
                    time_range = f"{times.min().strftime('%Y-%m-%d %H:%M:%S')} ~ {times.max().strftime('%Y-%m-%d %H:%M:%S')}"
                else:
                    time_range = ''
                file_stats.append({
                ...
                if 'Time' in df_tmp.columns:
                    for t in df_tmp['Time']:
                        ...
                        if pt:
                            times.append(pt)
                if times:
                    time_range = f"{min(times).strftime('%Y-%m-%d %H:%M:%S')} ~ {max(times).strftime('%Y-%m-%d %H:%M:%S')}"
                else:
                    time_range = ''
                file_stats.append({
                    '文件名': os.path.basename(f),
    ...
    for op in ['电信', '联通']:
        ...
        operator_stats.append({
            '运营商': op,
            '语音文件数': len(df_file_stats[df_file_stats['运营商'] == op]) if len(df_file_stats) > 0 else 0,
            '通话轮次': call_rounds,
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.3")
