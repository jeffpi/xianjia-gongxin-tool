...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True):
    ...
    # ===== 服务小区平均RSRP提取（从RRC_MEAS_RPRT的Detail Info列）=====
    log("提取服务小区RSRP值...", 58)
    rsrp_records = []

    if detail_col and len(df_voice_raw) > 0 and 'MessageType' in df_voice_raw.columns:
        # 筛选Message Type为RRC_MEAS_RPRT的行
        rrc_mask = df_voice_raw['MessageType'].astype(str) == 'RRC_MEAS_RPRT'
        rrc_rows = df_voice_raw[rrc_mask]

        for idx, row in rrc_rows.iterrows():
            rsrp_val = _extract_serv_rsrp(row[detail_col])
