...
class UCMProcessor:
    ...
    def _match_hybrid(self, agg, params):
        ...
        # 4个参考时间戳业务的BIZ_CONFIG (V1.05: ftp_dl/ftp_ul改用RLC层)
        REF_BIZ = {
            'ftp_dl':  {'dir': 'dl', 'col': 'dl_rlc',  'clip': 1000},
            'ftp_ul':  {'dir': 'ul', 'col': 'ul_rlc',  'clip': 200},
            'store_s': {'dir': 'dl', 'col': 'dl_rlc',  'clip': 1000},
            'wx_l':    {'dir': 'ul', 'col': 'ul_rlc',  'clip': 200},
        }
        ...
        ws = wb['呼叫详情']
        ref = {'电信': {'ftp_dl': [], 'ftp_ul': [], 'store_s': [], 'wx_l': []},
               '联通': {'ftp_dl': [], 'ftp_ul': [], 'store_s': [], 'wx_l': []}}
        ...
        # 确定运营商
        fn = getattr(self, '_fn', '')
        operator = '联通' if ('联通' in fn or 'unicom' in fn.lower()) else '电信'
        ...
        for biz, cfg in REF_BIZ.items():
            for r in ref[operator].get(biz, []):
                ...
                # V1.05: store_s用时间聚合(每秒取所有CRNTI的col最大值)，解决CRNTI切换问题
                if biz == 'store_s':
                    time_max = wdf.groupby('time')[cfg['col']].max()
                    vals = time_max.clip(upper=cfg['clip']) if cfg['clip'] else time_max
                    if len(vals) == 0:
                        continue
                    calc_rate = vals.mean()
                else:
                    cdf = wdf[wdf['crnti'] == best_crnti]
                    vals = cdf[cfg['col']].clip(upper=cfg['clip']) if cfg['clip'] else cdf[cfg['col']]
                    if len(vals) == 0:
                        continue
                    calc_rate = vals.mean()
        ...
        # ===== Part B: auto段检测补充 store_l + wx_s =====
        auto_rounds = self._detect_auto_segments(agg, params)
        # 把auto_rounds中的store_l/wx_s按时间就近分配到round_results
        biz_auto = {k: [] for k in ['store_l', 'wx_s']}
        for rd in auto_rounds:
            for k in ['store_l', 'wx_s']:
                if k in rd:
                    biz_auto[k].append(rd[k])
        # 遍历round_results，将auto段按时间匹配
        # 为每个轮次建立时间段
        rd_time_ranges = []
    ...
    def _detect_auto_segments(self, agg, params):
        ...
        for s in merged:
            ...
            if d == '下行':
                if flow <= 50: continue
                if 50 < flow < 350: continue  # store_s — 参考时间戳已有
                if 350 <= flow < 600: continue  # ftp_dl — 参考时间戳已有
                # V1.05: store_l阈值调整（流量>=800或dur>=10或rate<600）
                if flow >= 1500:
                    key_segs.append({'key': 'store_l', **s}); continue
                if flow >= 800:
                    if dur >= 10 or rate < 600:
                        key_segs.append({'key': 'store_l', **s}); continue
                if dur >= 20 or rate < 400:
                    key_segs.append({'key': 'store_l', **s}); continue
                # 其余判为ftp_dl（跳过）
            else:
                if flow < 3: continue
    ...
    def _load_mmf_with_crnti(self, paths):
        ...
        # CA聚合：同(time, crnti)下MAC求和，RLC取max
        groups = {}
        for r in all_rows:
            key = (r['time'], r['crnti'])
    ...
    def _summarize(self, round_results: List[Dict], params: Dict) -> Dict:
        ...
        stats = {}
        # FTP下载 — 口径：RLC削峰含零均值(dl_rlc_clip) V1.05改用RLC层对齐工信部
        d = empty('FTP下载')
        d['测试次数'] = count('ftp_dl')
        pts = collect_points('ftp_dl')
        if len(pts):
            clip = pts['dl_rlc'].clip(upper=1000)  # V1.05: RLC口径削峰含零
            d['采样点数'] = int(len(pts))
        ...
        stats['FTP下载'] = d
        # FTP上传 — 口径：RLC削峰含零均值(ul_rlc_clip) V1.05改用RLC层对齐工信部
        d = empty('FTP上传')
        d['测试次数'] = count('ftp_ul')
        pts = collect_points('ftp_ul')
        if len(pts):
            clip = pts['ul_rlc'].clip(upper=200)  # V1.05: RLC口径削峰含零
            d['采样点数'] = int(len(pts))
            d['上行RLC平均吞吐率(Mbps)'] = smean(clip)
        ...
        stats['FTP上传'] = d
        # 应用商店_小包 — 口径：RLC含零均值(dl_rlc_clip)对齐V2.00
        # 应用商店_大包 — 口径：mac_nz（MAC非零均值）对齐输出结果文件
        for key, grp, mode in [('store_s', '应用商店_小包', 'rlc_clip'), ('store_l', '应用商店_大包', 'mac_nz')]:
            d = empty(grp)
            d['测试次数'] = count(key)
            ...
            if len(pts):
                if mode == 'rlc_clip':
                    clip = pts['dl_rlc'].clip(upper=1000)  # RLC削峰含零均值
                    d['采样点数'] = int(len(pts))
                    d['下行RLC平均吞吐率(Mbps)'] = smean(clip)
                elif mode == 'mac_nz':
        ...
        # 微信_大文件 — 口径：rlc_clip（RLC削峰含零均值）对齐V2.00
        for key, grp, mode in [('wx_s', '微信_小文件', 'mac_nz'), ('wx_l', '微信_大文件', 'rlc_clip')]:
            d = empty(grp)
            d['测试次数'] = count(key)
            pts = collect_points(key)
            durs = collect_durations(key)
            if len(pts):
                if mode == 'mac_nz':
                    ...
                    rlc = pts['ul_rlc'].clip(upper=200)  # RLC削峰含零均值
                    d['采样点数'] = int(len(pts))
    ...
    def generate_annotated(self, mmf_paths, plan: List[Dict], params: Dict, out_path: str) -> str:
        ...
        def _fill_biz_segment(ws, seg_start, seg_end, label, e_col, f_col, g_col, h_col, thin_border,
            ...
            # 确定计算口径和用到的列 (V1.05: FTP改用RLC层)
            biz_config = {
                'FTP下载':   {'col': 'Downlink RLC Throughput(bps)', 'clip': 1000, 'mode': 'clip_mean', 'desc': '下行RLC削峰(上限1000Mbps)含零均值'},
                'FTP上传':   {'col': 'Uplink RLC Throughput(bps)', 'clip': 200, 'mode': 'clip_mean', 'desc': '上行RLC削峰(上限200Mbps)含零均值'},
                '应用商店小包': {'col': 'Downlink RLC Throughput(bps)', 'clip': 1000, 'mode': 'clip_mean', 'desc': '下行RLC削峰(上限1000Mbps)含零均值'},
                '应用商店大包': {'col': 'Downlink MAC Throughput(bps)', 'clip': None, 'mode': 'mac_nz', 'desc': '下行MAC非零均值(去零后平均)'},
                '微信小文件': {'col': 'Uplink MAC Throughput(bps)', 'clip': None, 'mode': 'mac_nz', 'desc': '上行MAC非零均值(去零后平均)'},
                '微信大文件': {'col': 'Uplink RLC Throughput(bps)', 'clip': 200, 'mode': 'clip_mean', 'desc': '上行RLC削峰(上限200Mbps)含零均值'},
            }
            ...
            if ri is not None:
                ...
                if seg_data:
                    ...
                    orig_count = len(arr)
                    nonzero_count = np.sum(arr != 0)

                    if mode == 'clip_mean' and clip_val:
                        clipped = np.clip(arr, None, clip_val)
                        final_mean = np.mean(clipped)
                        clipped_count = np.sum(arr > clip_val)
                        comment_f = f"计算口径: {desc}\n" \
                                    f"原始数据列: {calc_col_h}\n" \
                                    f"采样点数: {orig_count}\n" \
                                    f"原始均值: {orig_mean:.3f} Mbps\n" \
                                    f"削峰阈值: {clip_val} Mbps\n" \
                                    f"削峰点数: {clipped_count} 个\n" \
                                    f"削峰后均值: {final_mean:.3f} Mbps"
                    elif mode == 'mac_nz':
                        nonzero_arr = arr[arr != 0]
                        final_mean = np.mean(nonzero_arr) if len(nonzero_arr) > 0 else 0
                        comment_f = f"计算口径: {desc}\n" \
                                    f"原始数据列: {calc_col_h}\n" \
                                    f"采样点数: {orig_count}\n" \
                                    f"原始均值: {orig_mean:.3f} Mbps\n" \
                                    f"非零点数: {len(nonzero_arr)}\n" \
                                    f"非零均值: {final_mean:.3f} Mbps"
                    else:
...
class ExcelExporter:
    ...
    def export(self, output_path: str, call_df: pd.DataFrame, stats: Dict):
        ...
        self._create_biz_overview_sheet(call_df)
    ...
    def _create_call_sheet(self, call_df: pd.DataFrame):
        ...
        for col, h in enumerate(CALL_HEADERS, 1):
            ...
            c.font = Font(bold=True, size=9); c.fill = gfill; c.border = self.thin_border
        ...
        for r, row in enumerate(call_df.itertuples(index=False), 3):
            for col, v in enumerate(row, 1):
                c = ws.cell(row=r, column=col, value='' if (v is None or (isinstance(v, float) and pd.isna(v))) else v)
                c.border = self.thin_border

    def _create_biz_overview_sheet(self, call_df: pd.DataFrame):
        """创建业务概览sheet：子业务行，E=指标名，F=数值，G=时长指标，H=时长数值，同业务合并居中"""
        ws = self.wb.create_sheet('业务概览')
        gfill = PatternFill(start_color='CCE5FF', end_color='CCE5FF', fill_type='solid')
        # 表头
        ...
        # 定义子业务映射：业务类型 -> (关键列前缀, 速率列名, 时长列名)
        # 从call_df中提取每行的业务信息
        # call_df列：CALL_HEADERS
        # 找到需要的关键列索引
        def ci(name):
            return CALL_IDX.get(name)
        ...
        def _write_row(row_idx, biz_name, st, et, rate_label, rate_val, dur_label='业务时长(秒)', dur_val=''):
            """写一行业务数据，返回下一行索引"""
        ...
        for biz_data in biz_data_list:
            row_idx = _write_row(row_idx, *biz_data)
