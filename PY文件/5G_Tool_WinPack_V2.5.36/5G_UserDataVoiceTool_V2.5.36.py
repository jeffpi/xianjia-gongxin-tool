...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None, biz_params=None):
    ...
    def _match_strict(s, biz):
        ...
        if biz == '应用商店小文件下载':
            return dl > ul and dl >= 200 and 2 <= d <= 4
        if biz == '微信大包发送':
    ...
    def _match_relaxed(s, biz):
        ...
        if biz == '应用商店小文件下载':
            return dl > ul and dl >= 50 and d >= 2
        if biz == '微信大包发送':
    ...
    for si, seg in enumerate(valid):
        ...
        # 其他业务：直接标记有流量的行（加子段修剪）
        # 子段修剪：掐掉首尾低于峰值10%的低活跃秒（统一裁剪I列）
        _nonftp_vals = df.loc[mm, col].fillna(0)
        if len(_nonftp_vals) > 1:
            _pk = _nonftp_vals.max()
            if _pk > 0:
                _th = max(_pk * 0.1, 5.0)
                _active_mask = _nonftp_vals >= _th
                if _active_mask.any():
                    _ai = _nonftp_vals.index[_active_mask]
                    mm = mm & (df.index >= _ai[0]) & (df.index <= _ai[-1])
        if not mm.any():
            continue
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("工信部数据、语音跟踪统计工具 V2.5.36_CC")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("工信部数据统计工具 V2.5.36_CC")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("工信部语音VQI工具 V2.5.36_CC")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            vs = QSplitter(Qt.Vertical); vs.setChildrenCollapsible(False)
            if sys.platform == 'win32':
                vs.setStyleSheet("QSplitter::handle:vertical { background: #c8c8c8; height: 3px; margin: 1px 0; }")
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.36_CC &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
