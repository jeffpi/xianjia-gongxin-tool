...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None):
    ...
    if not operators_list:
        ...
        operators_list = ['未知']
    log(f"识别到运营商: {', '.join(operators_list)}", 47)
    ...
    # 统一加载基准数据（用户传入或默认永川参考）
    _base_file_to_load = None
    if base_file and os.path.exists(base_file):
        _base_file_to_load = base_file
    else:
        _default_base = '/Users/sun/ClaudeCode/先甲工信部工具/输入文件/0720-重大/重大-永川参考数据.xlsx'
        if os.path.exists(_default_base):
            _base_file_to_load = _default_base

    if _base_file_to_load:
        try:
            # 自动检测sheet：优先整理后文件→呼叫详情→首个sheet
            _xls = pd.ExcelFile(_base_file_to_load, engine='calamine')
            _sheet_names = _xls.sheet_names
            if '整理后文件' in _sheet_names:
                _sheet = '整理后文件'
            elif '呼叫详情' in _sheet_names:
                _sheet = '呼叫详情'
            else:
                _sheet = _sheet_names[0]
            raw = pd.read_excel(_base_file_to_load, sheet_name=_sheet, engine='calamine')
            log(f"加载基准文件: {os.path.basename(_base_file_to_load)} [{_sheet}], {len(raw)}行", 53)
    ...
    with xlsxwriter.Workbook(_out_tmp, {'strings_to_numbers': False, 'nan_inf_to_errors': True, 'default_date_format': 'yyyy-mm-dd hh:mm:ss'}) as wb:
        ...
        for op in operators_list:
            ...
            # D/E列用 set_column 列级格式(替代逐格选格式)
            if n_col_detail > col_3_idx:
                ws.set_column(col_3_idx, col_3_idx, None, fmt_d)
            if n_col_detail > col_4_idx:
                ws.set_column(col_4_idx, col_4_idx, None, fmt_e)
            ...
            for i in range(n_detail):
                ...
                for ci in range(n_col_detail):
                    ...
                    if ci == 0:
                        ...
                        row_vals.append('-' if d_dash else (float(v) if not pd.isna(v) else ''))
                    elif ci == col_4_idx:
                        row_vals.append('-' if e_dash else (float(v) if not pd.isna(v) else ''))
                    else:
                        _add_val(row_vals, v)
            ...
            def _add_comments(sub_detail, row_offset):
                    ...
                    for cell_col in range(2, 8):  # 1-based col 2..7
                        ...
                        if cp in (0, 1):
                            ...
                            if cp == 2:
                                ...
                                sv = rates.sort_values(ascending=False); tn = max(1, int(len(sv) * 0.1))
                                mol = list(sv.head(tn)); sum_v = sum(mol)
                                sum_label = f"TOP10%求和={sum_v:.1f}"; cnt_label = f"TOP10%计数={len(mol)}"
                                op_cn = f"SUM(TOP10% rate) / COUNT(TOP10%)\n= 排序后前10%速率求和 / 前10%个数"
                ...
                for bi, biz in enumerate(sorted(DL_BIZ | UL_BIZ)):
                    ...
                    if len(ts2) > 0:
                        s1 = ts2.iloc[0]; p = s1
                        for t in ts2.iloc[1:]:
                            if (t - p).total_seconds() > 5: dur_vals.append((p - s1).total_seconds() + 1); s1 = t
                            p = t
                        dur_vals.append((p - s1).total_seconds() + 1)
                    sr = sub.index; r_start = sr[0] + 2; r_end = sr[-1] + 2
                    row_idx_excel = bi + row_offset
                    row0 = row_idx_excel - 1
                    df_sum_ref = df_summary_all if row_offset == 3 else df_summary_complete
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None):
            super().__init__()
        ...
        def _do_data_run(self):
            ...
            # 读取用户自定义的业务执行顺序
            biz_order = None
            if hasattr(self, 'biz_order_list'):
                biz_order = [self.biz_order_list.item(i).text() for i in range(self.biz_order_list.count())]
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.22_CC")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("5G用户级公共监控速率统计工具 V2.5.22_CC")
            ...
            self.ftp_duration_spin.setValue(25)  # 默认25秒
            ...
            # 业务执行顺序（上移/下移按钮调整顺序）
            gpl.addWidget(QLabel("业务执行顺序:"), 8, 0, 1, 3)
            self.biz_order_list = QListWidget()
            self.biz_order_list.setMaximumHeight(130)
            self.biz_order_list.setSelectionMode(QListWidget.SingleSelection)
            _default_biz_order = ['FTP上传', 'FTP下载', '微信小包发送', '微信大包发送', '应用商店小文件下载', '应用商店大文件下载']
            for _item_text in _default_biz_order:
                self.biz_order_list.addItem(_item_text)
            gpl.addWidget(self.biz_order_list, 9, 0, 1, 2)
            # 上移/下移按钮
            _btn_up = QPushButton("↑上移"); _btn_up.setMaximumWidth(60)
            _btn_down = QPushButton("↓下移"); _btn_down.setMaximumWidth(60)
            _btn_up.clicked.connect(lambda: self._move_biz_item(-1))
            _btn_down.clicked.connect(lambda: self._move_biz_item(1))
            _btn_layout = QVBoxLayout()
            _btn_layout.addWidget(_btn_up); _btn_layout.addWidget(_btn_down)
            gpl.addLayout(_btn_layout, 9, 2, 1, 1)
            ll.addWidget(gp)
            ...
            self.result_tabs.addTab(self.tbl_detail, "详细过程")
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("5G虚拟用户跟踪语音VQI工具 V2.5.22_CC")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            # 输入文件（选择/清空紧挨着，拖入框明显、高度翻倍）
            gf = QGroupBox("输入文件"); gfl = QVBoxLayout(gf)
            gfl.setContentsMargins(4, 4, 4, 4); gfl.setSpacing(2)
        ...
        def _move_biz_item(self, direction):
            """移动业务执行顺序列表中的选中项: direction=-1上移, 1下移"""
            row = self.biz_order_list.currentRow()
            if row < 0: return
            new_row = row + direction
            if new_row < 0 or new_row >= self.biz_order_list.count(): return
            item = self.biz_order_list.takeItem(row)
            self.biz_order_list.insertItem(new_row, item)
            self.biz_order_list.setCurrentRow(new_row)
        ...
        def show_result(self, path):
            try:
                ...
                detail_sheets = sorted([s for s in sheet_names if '-详细过程' in s and '语音' not in s])
                summary_sheets = sorted([s for s in sheet_names if '-汇总' in s and '语音' not in s])
                # 兼容旧格式sheet名
                if not detail_sheets and '数据业务-详细过程' in sheet_names:
                    detail_sheets = ['数据业务-详细过程']
                if not summary_sheets and '汇总' in sheet_names:
                    summary_sheets = ['汇总']
                # 加载第一个详细过程sheet到详细过程tab
                if detail_sheets:
                    sn = detail_sheets[0]
                    self.result_tabs.setTabText(0, sn)
                    self._fill_table(self.tbl_detail, wb[sn], filter_a1=True, path=path, sheet=sn)
                # 加载第一个汇总sheet到汇总tab
                if summary_sheets:
                    sn = summary_sheets[0]
                    self.result_tabs.setTabText(1, sn)
                    self._fill_table(self.tbl_summary, wb[sn], filter_a1=False, path=path, sheet=sn)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.22_CC &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-20</b></p>"
                "<ul>"
                "<li><b>V2.5.22_CC</b> 修复基准列J~P仅首行填充bug：MMF业务段构建时跳过补秒空行（不再将其作为段边界），确保段范围覆盖段内所有行（含补秒行），使基准列逐行赋值不再被分割成单行段</li>\n"
                "<li><b>V2.5.21_CC</b> 分业务RLC速率汇总改为横表（联通/电信各独立列：业务/下行RLC吞吐率/上行RLC吞吐率，数值精确到0.01）；详细过程基准列J~P填充实际基准数据（速率类指标根据DL/UL方向填下行/上行RLC吞吐率(Mbps)，数值取永川逐秒RLC均值，时长类指标填业务时长(秒)及秒数）</li>\n"
                "<li><b>V2.5.20_CC</b> 新增「分业务RLC速率汇总」Sheet：按业务类型统计RLC吞吐率均值（上行类业务取Uplink RLC、下行类业务取Downlink RLC，bps→Mbps换算），短名称映射（微信大包发送→微信大包、应用商店大文件下载→应用宝大包等），跨运营商合并统计</li>\n"
                "<li><b>V2.5.19_CC</b> 基准对齐算法重构：时间搜索对齐(searchsorted+30秒容差)改为业务类型序列匹配（构建MMF业务段，贪心前向序列对齐，速率辅助校验），解决永川基准与MMF数据时间不同步导致75%不匹配率的问题</li>\n"
"<li><b>V2.5.18_CC</b> 速率辅助对齐：永川基准段计算RLC吞吐率平均值，基准对齐时用速率比率(0.3~3.0)辅助验证匹配正确性；可拖拽业务执行顺序：速率Tab参数设置区新增QListWidget，6业务类型可拖拽排序替代硬编码BIZ_ORDER；放宽时间对齐窗口：5秒→30秒容差，适应多秒级时间偏差</li>\n"
                "</ul>"
                "<p><b>2026-07-19</b></p>"
                "<ul>"
