                ...
                if seg_data:
                    ...
                    if mode == 'rlc_clip_nz' and clip_val:
                        ...
                        comment_f = f"计算口径: {desc}\n" \
                                    ...
                                    f"削峰阈值: {clip_val} Mbps (削峰{clipped_count}个点)\n" \
                                    f"去零后非零点数: {len(nz)}\n" \
                                    f"最终均值: {final_mean:.3f} Mbps"
                    elif mode == 'rlc_nz':
                        nz = arr[arr != 0]
                        final_mean = np.mean(nz) if len(nz) > 0 else 0
                        comment_f = f"计算口径: {desc}\n" \
                                    ...
                                    f"原始含零均值: {orig_mean:.3f} Mbps\n" \
                                    f"去零后均值: {final_mean:.3f} Mbps"
                    elif mode == 'mac_peak':
                        peak_val = float(np.max(arr))
                        comment_f = f"计算口径: {desc}\n" \
                                    f"原始数据列: {calc_col_h}\n" \
                                    f"采样点数: {orig_count}\n" \
                                    f"均值: {orig_mean:.3f} Mbps\n" \
                                    f"峰值(peak): {peak_val:.3f} Mbps"
                    elif mode == 'mac_mean':
                        comment_f = f"计算口径: {desc}\n" \
                                    f"原始数据列: {calc_col_h}\n" \
                                    f"采样点数: {orig_count}\n" \
                                    f"含零均值: {orig_mean:.3f} Mbps"
                    elif mode == 'rlc_mean':
                        comment_f = f"计算口径: {desc}\n" \
                                    f"原始数据列: {calc_col_h}\n" \
                                    f"采样点数: {orig_count}\n" \
