...
import urllib.request, urllib.error, hashlib, tempfile, shutil, time
...
# ==================== 统一鉴权 & 使用监控（5002 统一服务） ====================
AUTH_SERVER = 'http://47.109.101.79:5002'
TOOL_TYPE = 'xianjia'
TOOL_TYPE_NAME = '先甲工信部'

def _get_machine_code() -> str:
    """生成稳定机器指纹（MAC + 主机名 + 系统，SHA256 取前 16 位）"""
    try:
        mac = hex(uuid.getnode()) if hasattr(uuid, 'getnode') else 'unknown_mac'
    except Exception:
        mac = 'unknown_mac'
    hostname = platform.node() or 'unknown_host'
    raw = f'{mac}|{hostname}|{platform.system()}'
...
def _report_done_async(action: str, status: str = 'success', duration_ms: int = 0,
                        machine_code: str = None, tool_type: str = None):
    """异步（非阻塞）上报使用记录到 5002 统一监控"""
    try:
        mc = machine_code or _get_machine_code()
        tt = tool_type or TOOL_TYPE
        data = json.dumps({
            'machine_code': mc, 'tool_type': tt,
            'action': action, 'status': status,
            'duration_ms': duration_ms,
        }).encode('utf-8')
        req = urllib.request.Request(
            f'{AUTH_SERVER}/api/report_done',
            data=data,
...
class LicenseCheckThread(QThread):
    ...
    def run(self):
        try:
            ...
            url = f'{AUTH_SERVER}/api/check_license?code={mc}&tool_type={TOOL_TYPE}'
            resp = urllib.request.urlopen(url, timeout=8)
            result = json.loads(resp.read().decode())
        except Exception as e:
            result = {'allowed': True, 'error': str(e), 'machine_code': mc}
...
class UCMProcessor:
    ...
    def match(self, plan: List[Dict], params: Dict) -> Tuple[pd.DataFrame, Dict]:
        ...
        if params.get('match_mode') == 'auto':
            # 逐段分类模型: 分段→归类→按key收集,flat成round_results兼容_summarize
            segs_by_key = self._match_auto(agg, params, dl_min, ul_min)
            round_results = [{k: s} for k, segs in segs_by_key.items() for s in segs]
            round_results.sort(key=lambda rd: list(rd.values())[0]['start_t'])
            self._last_round_results = round_results
            # FTP呼叫详情: 下载段+上传段各一行,按时间排序
            call_rows=[]
            ftp_dl=sorted(segs_by_key.get('ftp_dl',[]),key=lambda x:x['start_t'])
            ftp_ul=sorted(segs_by_key.get('ftp_ul',[]),key=lambda x:x['start_t'])
            for i,s in enumerate(ftp_dl):
                r=['']*len(CALL_HEADERS);r[CALL_IDX['业务序号']]=i+1;r[CALL_IDX['业务类型']]='FTP'
                r[CALL_IDX['测试业务']]='FTPDownload';r[CALL_IDX['测试网络']]='NR5G'
                r[CALL_IDX['文件名']]=self._filename;r[CALL_IDX['发起时间']]=self._fmt_t(s['start_t'])
                r[CALL_IDX['完成时间']]=self._fmt_t(s['end_t']);r[CALL_IDX['下载平均速率(Mbps)']]=s['rate']
                call_rows.append(r)
            dl_cnt=len(call_rows)
            for i,s in enumerate(ftp_ul):
                r=['']*len(CALL_HEADERS);r[CALL_IDX['业务序号']]=dl_cnt+i+1;r[CALL_IDX['业务类型']]='FTP'
                r[CALL_IDX['测试业务']]='FTPUpload';r[CALL_IDX['测试网络']]='NR5G'
                r[CALL_IDX['文件名']]=self._filename;r[CALL_IDX['发起时间']]=self._fmt_t(s['start_t'])
                r[CALL_IDX['完成时间']]=self._fmt_t(s['end_t']);r[CALL_IDX['上传平均速率(Mbps)']]=s['rate']
                call_rows.append(r)
    ...
    # ---------- 自动识别匹配引擎（序列模式：分段→分类→按轮去重）----------
    def _match_auto(self, agg, params, dl_min, ul_min):
        """序列模式：分段→标记→合并→分组。电信测试固定顺序每轮一次每种业务。

        每轮模式(分段后gap模式):
          big_dl(9-12s,flow>300) -> [ACK上行10s,gap~9s] -> store_s(3-8s,flow 70-250) ->
          store_l(6-13s,flow>650) -> wx_s(2-4s,flow 5-30) -> wx_l(20-40s,flow>30)

        gap标记(下行视角):
          big_dl -> 下一段store_s: gap~55s(含ACK)
          store_s -> store_l: gap~24s
          store_l -> wx_l: gap~80s
        """
        n = len(agg)
        segs = []
        i = 0
        ...
        # === R1: 段特征初步标记(下行用big_dl暂存,上行排除ACK) ===
        for idx, s in enumerate(segs):
            d = s['direction']; flow = s['flow_mb']; dur = s['duration']
            if d == '下行':
                if flow < 30: s['key'] = None; continue
                if 70 <= flow <= 250 and dur <= 9:
                    s['key'] = 'store_s'; continue
                if flow > 300: s['key'] = 'big_dl'; continue
                if flow > 250: s['key'] = 'big_dl'; continue
                s['key'] = None
            else:  # 上行
                if flow < 3: s['key'] = None; continue
                # wx_s 优先识别: flow 3-30, dur<6s (短时小包,不是ACK)
                if 3 <= flow <= 30 and dur < 6:
                    s['key'] = 'wx_s'; continue
                # ACK: 上行dur 6-15s, 前面有大下行且gap小
                if dur >= 6 and dur <= 15 and idx > 0:
                    pk = segs[idx-1].get('key')
                    gp = (s['start_t'] - segs[idx-1]['end_t']).total_seconds()
                    if pk in ('big_dl', 'store_l', 'ftp_dl', 'store_s') and gp < 30:
                        s['key'] = None; continue  # ACK
                if flow > 30:
                    # 再检查ACK: 短上行(gap小)且前面紧邻大下行
                    if dur <= 15 and idx > 0:
                        pk = segs[idx-1].get('key')
                        gp = (s['start_t'] - segs[idx-1]['end_t']).total_seconds()
                        if pk in ('big_dl', 'store_l', 'ftp_dl', 'store_s') and gp < 30:
        ...
        # === R2: big_dl按rate区分为 ftp_dl 或 store_l ===
        # 电信基准: ftp_dl app层572,RLC~641; store_l app层1026,RLC~1150(削峰后563)
        # store_l: RLC rate>=850(严格,对应app层1026)且flow>650且dur<=15
        # 其余big_dl→ftp_dl
        for s in segs:
            if s.get('key') != 'big_dl': continue
            if s['rate'] >= 850 and s['flow_mb'] > 650 and s['duration'] <= 15:
                s['key'] = 'store_l'
            elif s['flow_mb'] > 1500 and s['duration'] > 50:
                # 超长ftp段(60s,6000MB)
                s['key'] = 'ftp_dl'
            else:
                s['key'] = 'ftp_dl'
        ...
        for idx, s in enumerate(segs):
            ...
            if len(s['points']) < 3: continue
            # 合并相邻同类: 如果下一个也是同样key且gap<120s(ftp_dl/store_l)或<60s(store_s)
            for dup_key, max_gap in [('store_s', 60), ('ftp_dl', 120), ('store_l', 120)]:
                if k == dup_key and idx < len(segs) - 1:
                    nk = segs[idx+1].get('key')
                    if nk == dup_key:
                        gap = (segs[idx+1]['start_t'] - s['end_t']).total_seconds()
                        if gap < max_gap:
                            skip_next = True; break
            merged.append(s)
        segs = merged

        # === R4: store_s 扩展前后1秒 ===
        for s in segs:
            if s.get('key') != 'store_s': continue
        ...
        for s in segs:
            ...
            if k is None: continue
            if len(s['points']) < 3: continue
            segs_by_key.setdefault(k, []).append(s)

        for k in ['ftp_dl','store_s','store_l','wx_s','wx_l']:
            print(f"DEBUG auto: {k} N={len(segs_by_key.get(k, []))}")

        return segs_by_key

    def _classify_seg(self, s, segs, idx):
        """归类(原段rate/flow/dur;对齐ALL_FTP_DETAIL基准反推)

        电信业务流程(每轮):
          ftp_dl(~10s,570-900MB) -> wait -> store_s(~3-8s,80-140MB) ->
          store_l(~6-13s,750-1500MB) -> wait -> ftp_dl(下一轮) -> wx_s(~2s,5-7MB) ->
          wx_l(~20-40s,170-350MB)

        关键区分:
          - store_s: 下行,flow 80-260MB,dur<10s
          - store_l: 下行,flow>650,dur<15,rate>850
          - ftp_dl:  下行,flow>300,典型500-900MB
          - wx_s:    上行,flow 5-10MB,dur<5s
          - wx_l:    上行,flow>30MB,典型170-350MB
        """
        d = s['direction']; flow = s['flow_mb']; dur = s['duration']; rate = s['rate']
        if d == '下行':
            if flow < 30: return None
            # store_s: flow 70-260MB,dur通常<10s
            if 70 <= flow <= 260 and dur <= 9:
                # 位置辅助:前有ftp_dl或后有store_l
                prev_key = segs[idx-1].get('key') if idx > 0 else None
                next_key = segs[idx+1].get('key') if idx < len(segs) - 1 else None
                if prev_key == 'ftp_dl' or next_key == 'store_l':
                    return 'store_s'
                if 80 <= flow <= 150:
                    return 'store_s'
                if 150 < flow <= 260 and rate < 350:
                    return 'store_s'
                return 'store_s'  # 兜底
            # store_l: flow大+dur短+高速
            if flow > 650 and dur <= 15:
                if rate > 850:
                    return 'store_l'
                if flow > 750:
                    return 'store_l'
            # ftp_dl: flow>300
            if flow > 300:
                if rate > 900 and flow > 650 and dur <= 15:
                    return 'store_l'  # store_l误入
                return 'ftp_dl'
            if flow > 260:
                return 'ftp_dl'
            return None
        else:  # 上行
            # wx_s: flow 5-30MB,dur<6s
            if 3 <= flow <= 30 and dur < 6:
                return 'wx_s'
            # wx_l: flow>30
            if flow > 30:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._start_license_check()  # 启动授权检查
    ...
    def start_processing(self):
        ...
        self._start_time = datetime.now()  # 记录处理开始时间
    ...
    def on_finished(self, success, message, call_df, stats):
        ...
        _report_done_async('process', 'success' if success else 'failed', duration_ms)
        if success:
    ...
    def _start_license_check(self):
        """启动时异步检查授权状态"""
        self._license_checker = LicenseCheckThread()
        self._license_checker.result_signal.connect(self._on_license_result)
        self._license_checker.start()

    def _on_license_result(self, result: dict):
        allowed = result.get('allowed', True)
        if not allowed:
            reason = result.get('reason', '未知原因')
            mc = result.get('machine_code', '?')
            QMessageBox.warning(self, '授权提示',
                f'当前设备（{mc}）未授权使用本工具。\n原因：{reason}\n\n请联系管理员。')
            self.close()
        else:
            self.log(f'[鉴权] 授权验证通过（设备: {result.get("machine_code", "?")}）')
