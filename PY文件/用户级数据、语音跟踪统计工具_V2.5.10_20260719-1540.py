...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    # 添加手机号列
    df_detail['手机号'] = df['_source_file'].apply(
        lambda sf: _lookup_phone(_extract_trace_id(
            next((info['full_path'] for fname, info in data_file_info.items() if fname == sf), '')
        ), phone_trace_map)
    )
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        ...
        # "所有业务"标题行
        ws_sum.merge_cells(start_row=2, start_column=1, end_row=2, end_column=9)
        ...
        for ci in range(1, 10): ws_sum.cell(row=2, column=ci).border = border
        ...
        ws_sum.merge_cells(start_row=comp_start, start_column=1, end_row=comp_start, end_column=9)
        ...
        for ci in range(1, 10): ws_sum.cell(row=comp_start, column=ci).border = border
...
def _build_anno_text(col_name, row_op, ri, df_voice_out, df_vqi, voice_records, vqi_records, rsrp_records, df_file_stats, df_operator_stats,
                     op_voice_cache=None, op_vqi_cache=None, op_rsrp_cache=None):
    """构建汇总-数据和语音表的详细批注（使用缓存避免重复过滤）"""
    if col_name == '呼叫建立时长(s)':
        # 使用缓存获取该运营商所有有值的呼叫建立时长
        ...
        vals = [float(vr['呼叫建立时长(s)']) for vr in op_vrs if vr.get('呼叫建立时长(s)') != '']
        if vals:
...
def _build_file_anno_text(col_name, fname, voice_records, vqi_records, rsrp_records,
    ...
    if col_name == '通话轮次':
        ...
        f_vrs = (fname_voice_cache or {}).get(fname, [vr for vr in voice_records if vr.get('_source_file') == fname])
        f_vrs = [vr for vr in f_vrs if vr.get('呼叫建立时长(s)') != '']
        vals = [float(vr['呼叫建立时长(s)']) for vr in f_vrs]
        if vals:
            nums = '/'.join([str(v) for v in vals])
            avg = round(sum(vals)/len(vals), 2)
        ...
        vals = [r['servRSRP值'] for r in f_rsrp]
        if vals:
            nums = '/'.join([str(v) for v in vals])
            avg = round(sum(vals)/len(vals), 2)
            return f"公式: 服务小区平均RSRP(dBm)\n数据来源: Detail Info -> servRSRP 列(RRC_MEAS_RPRT行)\n计算过程: 对 {fname} 的所有RSRP值取平均\n分子(各RSRP值): {nums}\n分母(RSRP数量): {len(vals)}\n计算结果: {avg}"
...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    if len(df_msg) > 0 and 'MessageType' in df_msg.columns and '运营商' in df_msg.columns:
        ...
        for op in operators_in_data:
            ...
            for ii, pos in enumerate(invite_positions):
                ...
                if len(bye) > 0:
                    bye_time = bye.iloc[0]['_t'] if '_t' in bye.columns else None
                    bye_time_raw = bye.iloc[0]['Time'] if 'Time' in bye.columns else None
                    bye_pos = bye.index[0]
                    if bye_time is not None:
                        # 在BYE附近1秒内找200（只在该运营商范围内）
                        # 使用iloc按位置索引，避免index不连续问题
                        try:
                            bye_loc = df_op.index.get_loc(bye_pos)
                        except KeyError:
                            bye_loc = -1
                        if bye_loc >= 0:
                            k_loc = bye_loc + 1
                            max_loc = len(df_op) - 1
                            while k_loc <= bye_loc + 20 and k_loc <= max_loc:
                                check_msg = str(df_op.iloc[k_loc]['MessageType'])
                                if check_msg == 'SIP_REQ_INVITE':
                                    break
                                if check_msg == 'SIP_RSP_200' and (df_op.iloc[k_loc]['_t'] - bye_time).total_seconds() <= 1:
                                    found_bye_200 = True
                                    last_time_raw = df_op.iloc[k_loc]['Time'] if 'Time' in df_op.columns else bye_time_raw
                                    break
                                k_loc += 1
                    if found_bye_200:
        ...
        if len(rrc_indices) > 0:
            ...
            for idx in rrc_indices:
                ...
                if m:
                    ...
                    rsrp_records.append({
                        '时间': time_vals[idx] if time_vals is not None else '',
                        '运营商': op_vals[idx] if op_vals is not None else '',
                        'servRSRP值': rsrp_val,
                        '_source_file': sf_vals[idx] if sf_vals is not None else ''
                    })
    ...
    for fname_base, f_info in voice_file_info.items():
        ...
        # 服务小区平均RSRP：从rsrp_records中筛选属于该文件的记录
    ...
    # ===== 按运营商统计 =====
    actual_ops_v = sorted(set(df_voice_out['运营商'].tolist() + (df_vqi['运营商'].tolist() if len(df_vqi) > 0 else [])))
    operator_stats = []
    for op in actual_ops_v:
        ...
        # 服务小区平均RSRP（按运营商）
        op_rsrp_records = [r for r in rsrp_records if r.get('运营商') == op]
        avg_rsrp_op = ''
        if op_rsrp_records:
            rsrp_vals = [r['servRSRP值'] for r in op_rsrp_records]
            avg_rsrp_op = round(sum(rsrp_vals) / len(rsrp_vals), 2)

        # 该运营商的文件列表
        op_file_stats = df_file_stats[df_file_stats['运营商'] == op] if len(df_file_stats) > 0 else pd.DataFrame()
        op_file_count = len(op_file_stats)
        # 文件名列表（换行分隔）
        op_file_names = '\n'.join(op_file_stats['文件名'].tolist()) if op_file_count > 0 else ''
        # 时间范围：每个文件的第一轮INVITE开始时间~最后一轮结束时间
        op_time_ranges = []
        for _, fs_row in op_file_stats.iterrows():
            fname = fs_row['文件名']
            f_vrs = [vr for vr in voice_records if vr.get('_source_file') == fname and vr.get('开始时间')]
            if f_vrs:
                first_start = f_vrs[0].get('开始时间', '')
                last_end = f_vrs[-1].get('结束时间', '')
                op_time_ranges.append(f"{first_start} ~ {last_end}")
            else:
                op_time_ranges.append(fs_row.get('时间范围', ''))
        op_time_range_str = '\n'.join(op_time_ranges) if op_time_ranges else ''

        # 手机号：该运营商所有文件的手机号（逗号分隔去重）
        op_phones = []
        for _, fs_row in op_file_stats.iterrows():
            if '手机号' in fs_row and fs_row['手机号']:
                op_phones.append(str(fs_row['手机号']))
        op_phone_str = '、'.join(sorted(set(op_phones))) if op_phones else ''
        ...
        operator_stats.append({
            ...
            '语音文件数': op_file_count,
            '文件名': op_file_names,
            '时间范围': op_time_range_str,
            '通话轮次': call_rounds,
            '平均呼叫建立时长(s)': avg_setup,
            '优质通话占比(MOS3.5)': quality_pct_op,
            '服务小区平均RSRP(dBm)': avg_rsrp_op,
            '下行前10%峰值速率': '',
            '下行均值速率': '',
            '上行前10%峰值速率': '',
            '上行均值速率': '',
    ...
    # 按运营商遍历（取voice和vqi中实际存在的运营商）
    actual_ops_summary = sorted(set(
        df_voice_out['运营商'].tolist() +
        (df_vqi['运营商'].tolist() if len(df_vqi) > 0 else [])
    ))
    for op in actual_ops_summary:
        op_voice = df_voice_out[df_voice_out['运营商'] == op]
        op_vqi = df_vqi[df_vqi['运营商'] == op] if len(df_vqi) > 0 else pd.DataFrame()

        # 该运营商的文件列表（按文件名统计表中筛选）
        op_file_stats = df_file_stats[df_file_stats['运营商'] == op] if len(df_file_stats) > 0 else pd.DataFrame()
        # 文件名：用换行符拼接所有文件名
        file_names = '\n'.join(op_file_stats['文件名'].tolist()) if len(op_file_stats) > 0 else ''
        # 时间范围：每个文件的第一轮INVITE开始时间~最后一轮结束时间
        op_file_time_ranges = []
        for _, fs_row in op_file_stats.iterrows():
            fname = fs_row['文件名']
            # 从voice_records中找该文件的第一轮和最后一轮的时间
            f_vrs = [vr for vr in voice_records if vr.get('_source_file') == fname and vr.get('开始时间')]
            if f_vrs:
                first_start = f_vrs[0].get('开始时间', '')
                last_end = f_vrs[-1].get('结束时间', '')
                op_file_time_ranges.append(f"{first_start} ~ {last_end}")
            else:
                op_file_time_ranges.append(fs_row.get('时间范围', ''))
        time_range_str_op = '\n'.join(op_file_time_ranges) if op_file_time_ranges else ''
        ...
        # 服务小区平均RSRP（按运营商汇总）
        op_rsrp_records_summary = [r for r in rsrp_records if r.get('运营商') == op]
        avg_rsrp_op_summary = ''
        if op_rsrp_records_summary:
            rsrp_vals_summary = [r['servRSRP值'] for r in op_rsrp_records_summary]
            avg_rsrp_op_summary = round(sum(rsrp_vals_summary) / len(rsrp_vals_summary), 2)

        # 手机号：该运营商所有文件的手机号（逗号分隔去重）
        op_phones_summary = []
        for _, fs_row in op_file_stats.iterrows():
            if '手机号' in fs_row and fs_row['手机号']:
                op_phones_summary.append(str(fs_row['手机号']))
        op_phone_summary_str = '、'.join(sorted(set(op_phones_summary))) if op_phones_summary else ''

        op_data_indicators = data_operator_indicators.get(op, {})
        summary_data.append({
            '运营商': op,
            '手机号': op_phone_summary_str,
            '文件名': file_names,
            '起止时间': time_range_str_op,
            '呼叫建立时长(s)': avg_setup,
            '优质通话占比(MOS3.5)': quality_pct_op,
            '服务小区平均RSRP(dBm)': avg_rsrp_op_summary,
    ...
    with pd.ExcelWriter(out, engine='xlsxwriter') as w:
        ...
        if merge_raw and len(df_raw_out) > 0:
            ...
            log(f"  语音原始数据共{len(df_raw_write)}行，全部写入", 76)

            ws_raw = workbook.add_worksheet('语音业务-原始文件')
            raw_cols = list(df_raw_write.columns)
            n_raw_cols = len(raw_cols)
            # 写表头
            for ci, col_name in enumerate(raw_cols):
                ws_raw.write(0, ci, str(col_name), hdr_fmt)
            # 写数据：用.values加速，write_row批量写入整行
            log(f"  写入{len(df_raw_write)}行x{n_raw_cols}列...", 77)
            raw_data = df_raw_write.values
            for ri in range(len(df_raw_write)):
                row_vals = []
                for ci in range(n_raw_cols):
                    val = raw_data[ri, ci]
                    if val is None or (isinstance(val, float) and pd.isna(val)):
                        row_vals.append('')
                    elif isinstance(val, (int, float)):
                ...
                ws_raw.write_row(ri + 1, 0, row_vals)
            ws_raw.set_column(0, 0, 22)
            ws_raw.set_column(1, 1, 22)
        ...
        if add_annotations:
            ...
            for sheet_name in ['汇总-数据和语音', '按文件名', '按运营商']:
                try:
                        ...
                        if isinstance(val, (int, float)) and not (isinstance(val, float) and pd.isna(val)):
                            ...
                            if sheet_name == '汇总-数据和语音':
                                ...
                                txt = _build_anno_text(col_name, row_op, ri, df_voice_out, df_vqi,
                                    voice_records, vqi_records, rsrp_records, df_file_stats, df_operator_stats,
                            ...
                            elif sheet_name == '按运营商':
                                op = df_anno.iloc[ri, 0]  # 运营商
                                txt = _build_op_anno_text(col_name, op, voice_records, vqi_records, rsrp_records, df_file_stats,
                                    _op_voice_cache, _op_vqi_cache, _op_rsrp_cache)
                            else:
                                txt = f"公式: {col_name}\n计算过程: 从原始数据统计\n\n数据来源: {col_name} 列"
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True, phone_trace_map=None):
            super().__init__()
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.10")
            ...
            # 手机号 ↔ Trace ID 映射
            self.phone_trace_map = []  # [{'phone': '138xxx', 'trace_id': '553'}, ...]
        ...
        def _lookup_phone(self, trace_id):
            """在phone_trace_map中精确查找trace_id对应的手机号"""
            if not trace_id or not self.phone_trace_map:
                return ''
            for entry in self.phone_trace_map:
                if entry['trace_id'] == trace_id:
                    return entry['phone']
            return ''

        def _build_phone_mapping_group(self, table_widget, text_edit, add_btn, clear_btn):
            ...
            add_btn.clicked.connect(lambda: self._add_phone_mapping_row(table_widget))
            vl.addWidget(add_btn)

            # 分隔说明
            vl.addWidget(QLabel("或拖入/粘贴映射文件（Excel两列: 手机号+TraceID, 或Txt两列Tab分隔, 列顺序自适应）："))

            # 拖入框
            text_edit.setMaximumHeight(80)
            text_edit.setPlaceholderText("拖入Excel/Txt文件，或粘贴文本内容...")
            ...
            clear_btn.clicked.connect(lambda: self._clear_phone_mapping(table_widget, text_edit))
            vl.addWidget(clear_btn)

            return gp

        def _add_phone_mapping_row(self, table_widget):
            """在映射表格中添加一行空行"""
            ...
            del_btn.clicked.connect(lambda: table_widget.removeRow(table_widget.currentRow()))
            table_widget.setCellWidget(row, 2, del_btn)

        def _clear_phone_mapping(self, table_widget, text_edit):
            """清空手机号映射（表格+文本框+内存）"""
            table_widget.setRowCount(0)
            text_edit.clear()
            self.phone_trace_map = []

        def _sync_phone_mapping(self):
            """从UI同步phone_trace_map到内存（两个tab分别调用）"""
            self.phone_trace_map = []
            # 速率tab
            for row in range(self.phone_table.rowCount()):
                phone_item = self.phone_table.item(row, 0)
                tid_item = self.phone_table.item(row, 1)
                phone = phone_item.text().strip() if phone_item else ''
                tid = tid_item.text().strip() if tid_item else ''
                if phone and tid:
                    self.phone_trace_map.append({'phone': phone, 'trace_id': tid})
            # VQI tab
            for row in range(self.vqi_phone_table.rowCount()):
                phone_item = self.vqi_phone_table.item(row, 0)
                tid_item = self.vqi_phone_table.item(row, 1)
                phone = phone_item.text().strip() if phone_item else ''
                tid = tid_item.text().strip() if tid_item else ''
                if phone and tid:
                    # 去重
                    if not any(e['trace_id'] == tid for e in self.phone_trace_map):
                        self.phone_trace_map.append({'phone': phone, 'trace_id': tid})
            # 解析文本框内容（拖入/粘贴的文件或文本）
            for te, tw in [(self.phone_textedit, self.phone_table),
                           (self.vqi_phone_textedit, self.vqi_phone_table)]:
                ...
                for line in lines:
                    ...
                    if os.path.isfile(line):
                        self._parse_mapping_file(line, tw)
                    else:
                        self._parse_mapping_line(line, tw)

            # === 关键修复：QTextEdit 内容已解析写入 QTableWidget，需要重新读取到 phone_trace_map ===
            for row in range(self.phone_table.rowCount()):
                phone_item = self.phone_table.item(row, 0)
                tid_item = self.phone_table.item(row, 1)
                phone = phone_item.text().strip() if phone_item else ''
        ...
        def _add_rows_to_table(self, rows, table_widget):
            ...
            for phone, tid in rows:
                if (phone, tid) not in existing:
                    ...
                    del_btn = QPushButton("删除")
                    del_btn.clicked.connect(lambda checked, r=row: table_widget.removeRow(r))
                    table_widget.setCellWidget(row, 2, del_btn)
        ...
        def _build_vqi_tab(self):
            ...
            self.vqi_cb_annotations.setChecked(False)
            ...
            gp_vqi_phone = self._build_phone_mapping_group(self.vqi_phone_table, self.vqi_phone_textedit, self.vqi_phone_add_btn, self.vqi_phone_clear_btn)
        ...
        def vqi_run(self):
            ...
            # 同步手机号映射
            self._sync_phone_mapping()
            self.vqi_thread = VqiWorkerThread(self.vqi_files, time_filter=time_filter, merge_raw=self.vqi_cb_merge_raw.isChecked(), add_annotations=self.vqi_cb_annotations.isChecked(), phone_trace_map=self.phone_trace_map)
        ...
        def run(self):
            ...
            # 同步手机号映射
            self._sync_phone_mapping()
            self.thread.phone_trace_map = self.phone_trace_map
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.10 &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
                "<b>联系方式:</b> 317827@qq.com</p><hr/>"
                "<h4>更新记录</h4>"
                "<p><b>2026-07-19</b></p>"
                "<ul>"
                "<li><b>V2.5.9</b> 修复运营商循环硬编码：3处`for op in ['电信', '联通']:`改为动态获取数据中实际运营商列表，支持'未知'运营商文件纳入统计；文件名更新时间戳为20260719</li>\n"
