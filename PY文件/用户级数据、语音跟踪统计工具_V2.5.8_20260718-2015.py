...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True):
...
def _build_anno_text(col_name, row_op, ri, df_voice_out, df_vqi, voice_records, vqi_records, rsrp_records, df_file_stats, df_operator_stats):
    ...
    if col_name == '呼叫建立时长(s)':
        # 找出该运营商所有有值的呼叫建立时长
        op_vrs = [vr for vr in voice_records if vr.get('运营商') == row_op and vr.get('呼叫建立时长(s)') != '']
        ...
        op_vqi = [r for r in vqi_records if r.get('运营商') == row_op]
        total = len(op_vqi)
        ...
        op_rsrp = [r for r in rsrp_records if r.get('运营商') == row_op]
...
def _build_file_anno_text(col_name, fname, voice_records, vqi_records, rsrp_records):
    """构建按文件名表的详细批注"""
    if col_name == '通话轮次':
        f_vrs = [vr for vr in voice_records if vr.get('_source_file') == fname]
        if f_vrs:
            rounds_detail = []
            for vr in f_vrs:
                rnd = vr.get('轮次', '')
        ...
        f_vrs = [vr for vr in voice_records if vr.get('_source_file') == fname and vr.get('呼叫建立时长(s)') != '']
        ...
        f_vqi = [r for r in vqi_records if r.get('_source_file') == fname]
        total = len(f_vqi)
        ...
        f_rsrp = [r for r in rsrp_records if r.get('_source_file') == fname]
...
def _build_op_anno_text(col_name, op, voice_records, vqi_records, rsrp_records, df_file_stats):
    """构建按运营商表的详细批注"""
    if col_name == '通话轮次':
        op_vrs = [vr for vr in voice_records if vr.get('运营商') == op]
        if op_vrs:
            rounds_detail = []
            for vr in op_vrs:
                rnd = vr.get('轮次', '')
        ...
        op_vrs = [vr for vr in voice_records if vr.get('运营商') == op and vr.get('呼叫建立时长(s)') != '']
        ...
        op_vqi = [r for r in vqi_records if r.get('运营商') == op]
        total = len(op_vqi)
        quality = sum(1 for r in op_vqi if r.get('是否优质(MOS>3.5)') == '是')
        if total > 0:
            return f"公式: 优质通话占比(MOS3.5)\n数据来源: MOS3.5计算明细 -> 是否优质(MOS>3.5) 列\n计算过程: 优质通话数 / 总通话数\n分子(优质通话数): {quality}\n分母(总通话数): {total}\n计算结果: {round(quality/total*100,2)}%"
        ...
        op_rsrp = [r for r in rsrp_records if r.get('运营商') == op]
        vals = [r['servRSRP值'] for r in op_rsrp]
        if vals:
            nums = '/'.join([str(v) for v in vals])
            avg = round(sum(vals)/len(vals), 2)
...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True, add_annotations=True):
                    ...
                    for ci in range(1, len(anno_cols)):
                        ...
                        if isinstance(val, (int, float)) and not (isinstance(val, float) and pd.isna(val)):
                            ...
                            if sheet_name == '汇总-数据和语音':
                                ...
                                txt = _build_anno_text(col_name, row_op, ri, df_voice_out, df_vqi, voice_records, vqi_records, rsrp_records, df_file_stats, df_operator_stats)
...
if GUI_OK:
    class WorkerThread(QThread):
        ...
        def __init__(self, files, qci_list, dl_clip, ul_clip, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True):
            super().__init__()
    ...
    class VqiWorkerThread(QThread):
        ...
        def __init__(self, files, time_filter=None, merge_raw=True, add_annotations=True):
            super().__init__()
            self.files = files
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.9")
        ...
        def vqi_run(self):
            ...
            self.vqi_thread = VqiWorkerThread(self.vqi_files, time_filter=time_filter, merge_raw=self.vqi_cb_merge_raw.isChecked(), add_annotations=self.vqi_cb_annotations.isChecked())
