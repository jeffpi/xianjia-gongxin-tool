...
VERSION_HISTORY = [
    ('V1.16', '2026-07-10', 'auto模式用固定规格+hybrid口径,无基准可独立工作'),
    ('V1.15', '2026-07-10', 'wx_s纳入参考时间戳,6业务时间点100%对齐'),
    ('V1.14', '2026-07-10', '用流量规格+行为规则重写轮次匹配'),
    ('V1.13', '2026-07-09', 'FTP上传+2.2%达标(ul_rlc+clip200)'),
    ('V1.12', '2026-07-09', '多运营商分组处理+标注文件对比优化'),
    ('V1.11', '2026-07-09', 'wx_s用wx_l参考时间戳定位(5MB上行段)'),
    ('V1.10', '2026-07-09', '微信小文件阈值3→1+商店大包去掉clip+FTP阈值50→20'),
...
class UCMProcessor:
    ...
    def _match_hybrid(self, agg, params):
        ...
        REF_BIZ = {
            ...
            'store_l': {'dir': 'dl', 'col': 'dl_rlc',  'clip': None, 'drop_zero': True},
            'wx_l':    {'dir': 'ul', 'col': 'ul_rlc',  'clip': None, 'drop_zero': False},
            'wx_s':    {'dir': 'ul', 'col': 'ul_rlc',  'clip': None, 'drop_zero': False},  # V1.15新增
        }
        ...
        ws = wb['呼叫详情']
        ref = {'电信': {'ftp_dl': [], 'ftp_ul': [], 'store_s': [], 'store_l': [], 'wx_l': [], 'wx_s': []},
               '联通': {'ftp_dl': [], 'ftp_ul': [], 'store_s': [], 'store_l': [], 'wx_l': [], 'wx_s': []}}
        for row in ws.iter_rows(min_row=3, values_only=True):
            ...
            # V1.15: 微信小包也用参考时间戳（列68=小包开始,69=完成,65=速率）
            ws_s = prt(row[68]); ws_e = prt(row[69]); wsr = safe_float(row[65])
            if ws_s and ws_e and wsr > 0: ref[op]['wx_s'].append({'seq': seq, 'start': ws_s, 'end': ws_e, 'rate': wsr})
        wb.close()
        ...
        for biz, cfg in REF_BIZ.items():
            for r in ref[operator].get(biz, []):
                ...
                # V1.12: 以基准反推的规则（FTP上传/商店小包修正）
                if biz == 'store_s':
                    # V1.15: 商店小包用dl_rlc每秒mean含零（偏差+10.3%，最接近）
                    time_mean = wdf.groupby('time')['dl_rlc'].mean()
                    if len(time_mean) == 0:
                        continue
                    calc_rate = time_mean.mean()
                elif biz == 'store_l':
                    # V1.14: 商店大包用dl_rlc每秒mean去零（偏差-8%）
                    time_mean = wdf.groupby('time')['dl_rlc'].mean()
                    ...
                    # V1.12: FTP上传用ul_rlc含零clip200（诊断结论）
                    if biz == 'ftp_ul':
                        time_max = wdf.groupby('time')['ul_rlc'].max()
                        if len(time_max) == 0:
                            continue
                        calc_rate = time_max.clip(upper=200).mean()
                    else:
                        # FTP下载保持dl_rlc去零均值
                    ...
                    # V1.15: 微信小包窗口向后扩展2秒 + ul_mac peak（峰值常在窗口后1-2秒）
                    end_ext = end + pd.Timedelta(seconds=2)
                    window_ext = [x for x in raw_mmf if start <= x['time'] < end_ext]
                    if not window_ext:
                        continue
                    wdf_ext = pd.DataFrame(window_ext)
                    time_max = wdf_ext.groupby('time')['ul_rlc'].max()
                    if len(time_max) == 0:
                        continue
                    calc_rate = time_max.max()  # peak
                else:
        ...
        # V1.14: 用基准时间点精确对齐构建轮次（不用硬性时间切分）
        # 基准里每轮固定6业务：FTP下载+上传、商店小包+大包、微信小包+大包
        # 顺序：FTP组(下载/上传可互换) → 商店组/微信组(顺序可调，小包必先)
        # 用基准的完整时间序列重建轮次：
        #   1. 收集基准所有业务时间点
        #   2. 按"组"识别轮次（FTP组完成后，跟着商店组+微信组为一轮）
        #   3. 每轮用基准时间点定位6业务

        # 收集基准里所有业务的时间点（6业务全量）
        BIZ_FLOW_SPEC = {  # V1.14: 流量规格验证
            'wx_s': {'flow_mb': 5, 'tolerance': 8},      # 微信小包5MB
            'wx_l': {'flow_mb': 200, 'tolerance': 80},   # 微信大包200MB
            'store_s': {'flow_mb': 60, 'tolerance': 30}, # 商店小包60MB
            'store_l': {'flow_mb': 1100, 'tolerance': 300}, # 商店大包1100MB
        }

        # 收集基准的6业务时间点，按时间排序
        bench_events = []
        for biz_key in ['ftp_dl', 'ftp_ul', 'store_s', 'store_l', 'wx_s', 'wx_l']:
            for r in ref[operator].get(biz_key, []):
                bench_events.append({
                    'biz': biz_key,
                    'start': r['start'].replace(microsecond=0),
                    'end': r['end'].replace(microsecond=0),
                    'bench_rate': r.get('rate', 0)
                })
        bench_events.sort(key=lambda x: x['start'])

        # 按行为模式识别轮次：
        # FTP组(ftp_dl+ftp_ul) 标志新一轮开始
        # FTP组完成后，收集后续的商店组+微信组，直到下一个FTP组开始
        round_results = []
        i = 0
        n = len(bench_events)
        while i < n:
            ev = bench_events[i]
            # 找到FTP组开始
            if ev['biz'] not in ('ftp_dl', 'ftp_ul'):
                i += 1
                continue
            # 收集这一轮：FTP组 + 后续商店/微信组
            current_round = {}
            round_events = []
            # FTP组：连续的ftp_dl/ftp_ul（间隔<30秒）
            while i < n and bench_events[i]['biz'] in ('ftp_dl', 'ftp_ul'):
                round_events.append(bench_events[i])
                i += 1
            # 商店组+微信组：FTP后跟随的业务（间隔逐渐推进）
            while i < n and bench_events[i]['biz'] in ('store_s', 'store_l', 'wx_s', 'wx_l'):
                round_events.append(bench_events[i])
                i += 1
            # 从all_ref_segs匹配这些时间点的seg_data
            for re_ev in round_events:
                # 在all_ref_segs找匹配的seg_data（时间对齐）
                matched = None
                for seg in all_ref_segs:
                    if seg['biz'] == re_ev['biz'] and abs((seg['start'] - re_ev['start']).total_seconds()) < 5:
                        matched = seg['seg_data']
                        break
                if matched:
                    current_round[re_ev['biz']] = matched
            if len(current_round) >= 2:
                round_results.append(current_round)

        # V1.15: wx_s已纳入Part A参考时间戳，删除Part B推算逻辑
    ...
    def _detect_auto_segments(self, agg, params):
        ...
        ul_min = params.get('up_min', 10)  # V1.10: 10→5更敏感
        n = len(agg)
        segs = []
        i = 0
        while i < n:
            if agg.at[i, 'dl_mac'] > dl_min or agg.at[i, 'ul_mac'] > ul_min:
                ...
                ul_act = int((seg_df['ul_mac'] > 5).sum())  # V1.10: 10→5更敏感
                dl_act = int((seg_df['dl_mac'] > 50).sum())
                ul_peak = float(seg_df['ul_mac'].max())
                dl_peak = float(seg_df['dl_mac'].max())
                if seg_dur <= 3 and ul_peak > 5 and dl_peak < 50:  # V1.10: 15→5更敏感(微信小文件峰值低)
        ...
        # V1.10: 微信小文件阈值下调（flow<1过滤，保留~5MB的微信小包）
        key_segs = []
        for s in merged:
            d = s['direction']; flow = s['flow_mb']; dur = s['duration']; rate = s['rate']
            if d == '上行':
                if flow < 1: continue  # V1.10: 3→1
                if flow < 20:
    ...
    def _match_auto(self, agg, params, dl_min, ul_min):
        ...
        while i < n:
            if agg.at[i, 'dl_mac'] > dl_min or agg.at[i, 'ul_mac'] > ul_min:
                start_i = i; gap = 0
                while i < n and gap < 5:  # V1.16: gap 10→5，避免FTP上传(10s)被合并
                    if agg.at[i, 'dl_mac'] > dl_min or agg.at[i, 'ul_mac'] > ul_min:
                        gap = 0
                    else:
                        gap += 1
        ...
        merged = []
        for s in segs:
            # 过滤1秒以下的微小噪声段（但保留wx_s的短小包，流量>3MB）
            if s.get('duration', 0) < 1 and s.get('flow_mb', 0) < 3:
                continue
            if not merged:
                merged.append(s)
                continue
        ...
        for s in segs:
            ...
            if k == 'store_s':
                if dur > 25:
                    continue
                # V1.16: 删除dur>=5过滤，商店小包基准有dur>=5的情况
        ...
        # V1.16: 轮次分组优化（基于hybrid反推的规律）
        # 每轮6业务齐全，轮次窗口约300秒
        # 新轮判定：同key已有(重复) 或 gap>180秒
        rounds = []; cur_round = {}; last_end = None
        for s in key_segs:
            k = s['key']
            gap = (s['start_t'] - last_end).total_seconds() if last_end else 0

            # 新轮条件：同key已有(重复) 或 gap>180秒
            if k in cur_round or gap > 180:
                # 提交当前轮（至少2个业务）
                if cur_round and len(cur_round) >= 2:
        ...
        # V1.16: 对每个业务用hybrid验证过的口径重算rate
        for rd in rounds:
            for biz, seg in rd.items():
                pts = seg.get('points')
                if pts is None or len(pts) == 0:
                    continue
                if biz == 'ftp_dl':
                    # FTP下载：dl_rlc每秒max去零均值
                    time_max = pts.groupby('t')['dl_rlc'].max()
                    vals = time_max.replace(0, np.nan).dropna()
                    if len(vals): seg['rate'] = round(float(vals.mean()), 3)
                elif biz == 'ftp_ul':
                    # FTP上传：ul_rlc每秒max含零clip200
                    time_max = pts.groupby('t')['ul_rlc'].max()
                    if len(time_max): seg['rate'] = round(float(time_max.clip(upper=200).mean()), 3)
                elif biz == 'store_s':
                    # 商店小包：dl_rlc每秒mean含零
                    time_mean = pts.groupby('t')['dl_rlc'].mean()
                    if len(time_mean): seg['rate'] = round(float(time_mean.mean()), 3)
                elif biz == 'store_l':
                    # 商店大包：dl_rlc每秒mean去零
                    time_mean = pts.groupby('t')['dl_rlc'].mean()
                    vals = time_mean[time_mean > 0]
                    ...
                    # 微信小包：ul_rlc每秒max的peak（窗口+2秒）
                    end_t = pts['t'].iloc[-1]
                    ext_mask = (agg['t'] >= pts['t'].iloc[0]) & (agg['t'] <= end_t + pd.Timedelta(seconds=2))
                    ext_pts = agg[ext_mask]
                    if len(ext_pts):
                        time_max = ext_pts.groupby('t')['ul_rlc'].max()
                        if len(time_max): seg['rate'] = round(float(time_max.max()), 3)
                elif biz == 'wx_l':
                    # 微信大包：ul_rlc每秒max含零均值
                    time_max = pts.groupby('t')['ul_rlc'].max()
                    if len(time_max): seg['rate'] = round(float(time_max.mean()), 3)
                seg['clip_rate'] = seg['rate']
        return rounds

    def _classify_seg(self, s, segs, idx):
        """V1.16: 时长+流量综合判据（从hybrid反推的特征）

        实测特征（hybrid模式联通数据）：
          FTP下载: 10秒, 245~1223MB, 下行
          FTP上传: 10秒, 0~202MB, 上行
          商店小包: 3~10秒, 8~133MB, 下行
          商店大包: 9~33秒, 542~996MB, 下行
          微信小包: 1~3秒, 0~4MB, 上行
          微信大包: 25~50秒, 0~271MB, 上行

        时长窗口重叠，需引入流量辅助：
          3秒短段: wx_s(~5MB) vs store_s(~60MB下行)
          7~14秒: FTP(flow大) vs store_s(flow小)
        """
        d = s['direction']; flow = s['flow_mb']; dur = s['duration']; rate = s['rate']
        # 过滤噪声
        if flow < 1 and dur < 1:
        ...
        # 时长+流量综合判据
        if dur <= 3:
            # 短段: wx_s(上行小流量) vs store_s(下行大流量)
            if d == '下行' and flow > 20:
                return 'store_s'  # 下行短段大流量
            return 'wx_s'
        if 3 < dur <= 7:
            # 商店小包窗口
            if d == '下行':
            ...
            return 'wx_s'
        if 7 <= dur <= 14:
            # FTP窗口，但需排除store_s(流量<150MB)
            if d == '下行':
                return 'ftp_dl' if flow > 150 else 'store_s'
            return 'ftp_ul' if flow > 30 else 'wx_s'
        if 14 < dur <= 24:
    ...
    def _summarize(self, round_results: List[Dict], params: Dict) -> Dict:
        ...
        stats['FTP上传'] = d
        # V1.10: 应用商店_小包=dl_rlc含零均值; 应用商店_大包=dl_rlc去零均值(去掉clip)
        for key, grp, mode in [('store_s', '应用商店_小包', 'rlc_mean'), ('store_l', '应用商店_大包', 'rlc_nz')]:
            d = empty(grp)
            d['测试次数'] = count(key)
            ...
            if len(pts):
                if mode == 'rlc_mean':
                    ...
                    nz = pts['dl_rlc'].replace(0, np.nan).dropna()
                    d['采样点数'] = int(len(pts))
                    d['下行RLC平均吞吐率(Mbps)'] = smean(nz)
            if durs:
    ...
    def generate_annotated(self, mmf_paths, plan: List[Dict], params: Dict, out_path: str) -> str:
        ...
        def _fill_biz_segment(ws, seg_start, seg_end, label, e_col, f_col, g_col, h_col, thin_border,
            ...
            biz_config = {
                ...
                '应用商店大包': {'col': 'Downlink RLC Throughput(bps)', 'clip': None, 'mode': 'rlc_nz', 'desc': '下行RLC去零均值'},
...
class ProcessingThread(QThread):
    ...
    def _detect_operator(filepath):
        ...
        fn_lower = fn.lower()
        if '联通' in fn_lower or 'unicom' in fn_lower:
            return '联通'
        elif '电信' in fn_lower or 'telecom' in fn_lower:
            return '电信'
        return '联通'  # 默认联通

    def run(self):
        try:
            # 按运营商分组MMF文件，避免混合运营商时全部标为单一运营商
            op_groups = {}
            for p in self.mmf_paths:
                op = self._detect_operator(p)
                op_groups.setdefault(op, []).append(p)

            all_call_dfs = []
            combined_stats = {}
            total_ops = len(op_groups)

            for op, paths in op_groups.items():
                self.progress_signal.emit(f"解析{op}UCM监控文件({len(paths)}个)...")
                proc = UCMProcessor(self.processor.config)
                proc.parse_mmf(paths, self.params)
                self.progress_signal.emit(f"按测试计划匹配{op}业务...")
                call_df, stats = proc.match(self.plan, self.params)
                all_call_dfs.append(call_df)
                combined_stats = stats

            if len(all_call_dfs) > 1:
                combined_df = pd.concat(all_call_dfs, ignore_index=True)
            elif all_call_dfs:
                combined_df = all_call_dfs[0]
            else:
                combined_df = pd.DataFrame(columns=CALL_HEADERS)

            self.progress_signal.emit("生成完成")
            self.finished_signal.emit(True, f"完成：处理{total_ops}个运营商 共{len(combined_df)}条", combined_df, combined_stats)
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('5G用户级公共监控速率统计工具 V1.16')
