...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=20, merge_raw=True, add_annotations=True, phone_trace_map=None):
    ...
    # 添加OC列
    for c in OC:
        detail_dict[c] = df[c].values if c in df.columns else [None] * len(df)
    detail_dict['businessType'] = df['业务识别'].values
    # 初始化基准列(后续填充)
    for bc in base_cols:
        detail_dict[bc] = [None] * len(df)
    # 添加_t列
    detail_dict['_t'] = df['_t'].values
    # 预构建文件名到路径的映射 + trace_id到手机号的映射(性能优化)
    file_to_path = {fname: info['full_path'] for fname, info in data_file_info.items()}
    trace_to_phone = {}
    if phone_trace_map:
        for entry in phone_trace_map:
            trace_to_phone[entry['trace_id']] = entry['phone']
    # 添加手机号列(使用预构建映射,避免逐行遍历字典)
    detail_dict['手机号'] = [
        trace_to_phone.get(_extract_trace_id(file_to_path.get(sf, '')), '')
        for sf in df['_source_file'].values
    ]
    # 添加special_cols
    for sc in special_cols:
        if sc in df.columns:
            detail_dict[sc] = df[sc].values
    # 添加其他列
    for c in df.columns:
        if c not in detail_dict and c not in exclude_cols:
            detail_dict[c] = df[c].values
