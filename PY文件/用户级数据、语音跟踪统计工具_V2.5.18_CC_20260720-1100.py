...
def process(files, qci_list=None, dl_clip=1000, ul_clip=200, callback=None, cancel_check=None, progress_cb=None, base_file=None, time_filter=None, ftp_duration=25, merge_raw=False, add_annotations=True, phone_trace_map=None, biz_order=None):
    ...
    if base_file and os.path.exists(base_file):
        try:
            try:
                user_base = pd.read_excel(base_file, sheet_name='呼叫详情', engine='calamine')
            except:
                user_base = pd.read_excel(base_file, engine='calamine')
            if user_base is not None and len(user_base) > 0:
                log(f"使用用户基准文件: {os.path.basename(base_file)}", 53)
                for op in operators_list:
                    per_operator_bases[op] = user_base
            else:
                for op in operators_list:
        ...
        if not os.path.exists(default_base):
            ...
            try:
                raw = pd.read_excel(default_base, sheet_name='整理后文件', engine='calamine')
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.19_CC")
        ...
        def _build_rate_tab(self):
            ...
            ll.setSpacing(4)
            title = QLabel("5G用户级公共监控速率统计工具 V2.5.19_CC")
            ...
            self.biz_order_list.setDragDropMode(QListWidget.InternalMove)
            self.biz_order_list.setDefaultDropAction(Qt.MoveAction)
            self.biz_order_list.setDragEnabled(True)
            self.biz_order_list.setMaximumHeight(150)
            self.biz_order_list.setSelectionMode(QListWidget.SingleSelection)
            _default_biz_order = ['FTP上传', 'FTP下载', '微信小包发送', '微信大包发送', '应用商店小文件下载', '应用商店大文件下载']
            for _item_text in _default_biz_order:
                _item = QListWidgetItem(_item_text)
                _item.setFlags(_item.flags() | Qt.ItemIsDragEnabled | Qt.ItemIsDropEnabled)
                self.biz_order_list.addItem(_item)
            gpl.addWidget(self.biz_order_list, 9, 0, 1, 3)
            ll.addWidget(gp)
        ...
        def _build_vqi_tab(self):
            ...
            ll.setContentsMargins(4, 4, 4, 4)
            ll.setSpacing(4)
            title = QLabel("5G虚拟用户跟踪语音VQI工具 V2.5.19_CC")
            title.setFont(self._font(2)); title.setAlignment(Qt.AlignCenter); ll.addWidget(title)
            # 输入文件（选择/清空紧挨着，拖入框明显、高度翻倍）
            gf = QGroupBox("输入文件"); gfl = QVBoxLayout(gf)
            gfl.setContentsMargins(4, 4, 4, 4); gfl.setSpacing(2)
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>版本:</b> V2.5.19_CC &nbsp; <b>开发者:</b> 孙晓军 &nbsp; "
