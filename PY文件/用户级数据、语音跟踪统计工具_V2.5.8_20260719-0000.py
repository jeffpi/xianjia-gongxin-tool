...
def process_vqi(files, callback=None, cancel_check=None, progress_cb=None, time_filter=None, time_range=None, merge_raw=True):
    ...
    if detail_col and len(df_voice_raw) > 0:
        # 向量化提取：避免逐行iterrows
        detail_series = df_voice_raw[detail_col].astype(str)
        # 使用str.extract批量提取
        vqi_matches = detail_series.str.extract(_VQI_RE, expand=False)[0]
        vqi_valid = pd.to_numeric(vqi_matches, errors='coerce')
        # 排除65535
        vqi_valid = vqi_valid[vqi_valid != 65535]
        vqi_valid = vqi_valid.dropna()

        if len(vqi_valid) > 0:
            # 批量构建vqi_records
            for idx in vqi_valid.index:
                vqi = int(vqi_valid[idx])
                mos = vqi / 100.0
                is_quality = '是' if mos > 3.5 else '否'
                vqi_records.append({
                    '时间': df_voice_raw.loc[idx, 'Time'],
                    '运营商': df_voice_raw.loc[idx, '运营商'],
                    'DlE2eVqi原始值': vqi,
                    'MOS值': round(mos, 2),
                    '是否优质(MOS>3.5)': is_quality,
                    '_source_file': df_voice_raw.loc[idx, '_source_file']
                })
    ...
    # ===== 服务小区平均RSRP提取（从RRC_MEAS_RPRT的Detail Info列）向量化优化 =====
    log("提取服务小区RSRP值...", 58)
    rsrp_records = []

    if detail_col and len(df_voice_raw) > 0 and 'MessageType' in df_voice_raw.columns:
        # 向量化筛选RRC_MEAS_RPRT行
        rrc_mask = df_voice_raw['MessageType'].astype(str) == 'RRC_MEAS_RPRT'
        rrc_indices = rrc_mask[rrc_mask].index

        if len(rrc_indices) > 0:
            # 批量提取RSRP
            rrc_detail = df_voice_raw.loc[rrc_indices, detail_col].astype(str)
            rsrp_matches = rrc_detail.str.extract(_RSRP_RE, expand=False)

            for i, idx in enumerate(rrc_indices):
                val1_str = rsrp_matches.iloc[i, 0]
                val2_str = rsrp_matches.iloc[i, 1]

                if pd.notna(val1_str):
                    val1 = int(val1_str)
                    val2 = int(val2_str) if pd.notna(val2_str) else None
                    rsrp_val = max(val1, val2) if val2 is not None else val1
                    rsrp_records.append({
                        '时间': df_voice_raw.loc[idx, 'Time'],
                        '运营商': df_voice_raw.loc[idx, '运营商'],
                        'servRSRP值': rsrp_val,
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        ...
        # 预定义各sheet的格式化参数，避免重复代码
        sheet_formats = [
            ('语音指标统计', True, None, 20, None),
            ('MOS3.5计算明细', True, 'green_fill', 22, None),
            ('汇总-数据和语音', True, 'blue_fill', 18, 'annotate'),
            ('按文件名', True, None, 20, 'annotate'),
            ('按运营商', True, None, 20, 'annotate'),
        ]

        for sheet_name, freeze, last_row_fill, col_width, annotate in sheet_formats:
            try:
            ...
            for ri in range(2, ws.max_row + 1):
                for ci in range(1, ws.max_column + 1):
                    ...
                    cell.alignment = center; cell.border = border
                    if last_row_fill == 'green_fill' and ri == ws.max_row:
                        cell.fill = green_fill
                    if sheet_name == '语音指标统计' and ci == 5:
                        cv = cell.value
                        if cv is None or str(cv) == '' or str(cv) == 'nan':
                            cell.comment = _Comment("无RSP_180", "Tool")
                    if sheet_name == '汇总-数据和语音' and ri == 2:
                        cell.fill = PatternFill(start_color='CCE5FF', end_color='CCE5FF', fill_type='solid')
            if col_width:
                for col_idx in range(1, ws.max_column + 1):
                    ws.column_dimensions[_gcl(col_idx)].width = col_width
            if freeze:
                ws.freeze_panes = 'A2'
            if annotate:
                for ri in range(2, ws.max_row + 1):
                    for ci in range(2, ws.max_column + 1):
                        ...
                        if cv is not None and isinstance(cv, (int, float)):
                            col_name = ws.cell(row=1, column=ci).value or f'列{ci}'
                            txt = f"公式: {col_name}\n计算过程: 从原始数据统计\n\n数据来源: {col_name} 列"
...
if GUI_OK:
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("用户级数据、语音跟踪统计工具 V2.5.8")
        ...
        def about(self):
            ...
            te.setHtml(
                ...
                "<p><b>2026-07-18</b></p>"
                "<ul>"
"<li><b>V2.5.8</b> 性能优化：基准对齐使用二分查找(searchsorted)替代向量化abs计算，大幅提升大文件处理速度；新增服务小区平均RSRP统计(从RRC_MEAS_RPRT提取servRSRP，双值取最大)；修复RSRP正则支持单值无逗号格式</li>\n"
